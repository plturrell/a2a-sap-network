"""
Main entry point for Agent 5 - QA Validation Agent
A2A Microservice for data quality validation and QA checks
"""
...
if AGENT_AVAILABLE:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Initialize agent
        base_url = os.getenv("A2A_AGENT_URL", "http://localhost:8005")
        agent_manager_url = os.getenv("A2A_MANAGER_URL", "http://localhost:8000")
        downstream_agent_url = os.getenv("A2A_DOWNSTREAM_URL", "http://localhost:8006")
    ...
    app = A2AMicroservice(
        ...
        description="A2A Microservice for data quality validation and quality assurance checks",
        version="3.0.0",
        lifespan=lifespan
    ).app
else:
    ...
    app = FastAPI(
        title="Agent 5 - QA Validation",
        description="A2A Microservice for data quality validation (Simple Mode)",
        version="3.0.0"
    )
...
if AGENT_AVAILABLE:
    ...
    async def get_stats():
        ...
        return {
            ...
            "validation_rules": agent_instance.validation_rules,
            "status": "active" if agent_instance.is_registered else "inactive"
        }

    @app.post("/validate")
    async def validate_quality(request: dict):
        """Endpoint for direct QA validation requests"""
        if not agent_instance:
            return {"error": "Agent not initialized"}
        ...
        try:
            ...
            # Process validation
            result = await agent_instance.handle_qa_request(message, context_id)
            return result
...
async def root():
    """Root endpoint"""
    return {
        "message": "Agent 5 - QA Validation Service",
        "version": "3.0.0",
