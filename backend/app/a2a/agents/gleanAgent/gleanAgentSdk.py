...
class GleanAgent(A2AAgentBase, BlockchainIntegrationMixin):
    ...
    def __init__(self, base_url: str = None):
        ...
        self.trust_info = initialize_agent_trust(
            ...
            agent_type="analysis",
            capabilities=["code_analysis", "linting", "testing", "security_scan"]
        )
    ...
    async def _run_javascript_linters_batch(self, files: List[Path], directory: str) -> Dict[str, Any]:
        """Run JavaScript linters on a batch of files"""
        issues = []
        linter_results = {}
        
        # Check which JS linters are available
        available_linters = []
        ...
        if not available_linters:
            logger.warning("No JavaScript linters available")
            return {"issues": [], "linter_results": {}}
        
        for linter in available_linters:
            try:
                if linter == "eslint":
                    result = await self._run_eslint(files, directory)
                elif linter == "jshint":
                    result = await self._run_jshint(files, directory)
    ...
    async def _run_typescript_linters_batch(self, files: List[Path], directory: str) -> Dict[str, Any]:
        """Run TypeScript linters on a batch of files"""
        # For now, use ESLint for TypeScript files too
        return await self._run_javascript_linters_batch(files, directory)
        ...
        return {
            ...
            "issues_by_severity": severity_counts,
            "issues_by_type": type_counts,
            "issues": [asdict(issue) for issue in issues[:100]]  # Limit to first 100
        }
