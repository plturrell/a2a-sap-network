...
class DataManagerAgentSDK(A2AAgentBase):
    ...
    async def _initialize_storage_clients(self):
        ...
        try:
            ...
            if SQLITE_AVAILABLE and self.storage_config.get("sqlite"):
                try:
                    sqlite_config = self.storage_config["sqlite"]
                    db_path = sqlite_config.get("path", "a2a_data.db")
                    ...
                    cursor.execute('''
                        CREATE TABLE IF NOT EXISTS a2a_data_records (
                    ...
                    # Create indexes
                    cursor.execute("CREATE INDEX IF NOT EXISTS idx_agent_id ON a2a_data_records(agent_id)")
                    cursor.execute("CREATE INDEX IF NOT EXISTS idx_context_id ON a2a_data_records(context_id)")
                    cursor.execute("CREATE INDEX IF NOT EXISTS idx_data_type ON a2a_data_records(data_type)")
                    
                    self.sqlite_connection.commit()
                    
                    self.storage_clients["sqlite"] = self._sqlite_client
                    logger.info("✅ SQLite storage client initialized as fallback")
    ...
    def _sqlite_client(self, operation: str, *args, **kwargs):
        """SQLite storage client implementation"""
        return {"operation": operation, "backend": "sqlite", "connection": self.sqlite_connection}
    ...
    async def _store_data(self, data: Dict[str, Any], storage_backend: str) -> str:
        ...
        if storage_backend == "filesystem":
            ...
            try:
                ...
                logger.error(f"HANA storage failed: {e}, falling back to SQLite")
                if "sqlite" in self.storage_clients:
                    return await self._store_data(data, "sqlite")
                else:
                    return await self._store_data(data, "filesystem")
                    
        elif storage_backend == "sqlite" and self.sqlite_connection:
            try:
                # Prepare record
                cursor = self.sqlite_connection.cursor()
                cursor.execute('''
                    INSERT OR REPLACE INTO a2a_data_records 
                    (record_id, agent_id, context_id, data_type, data, metadata, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    data_id,
                    data.get("agent_id", "data_manager"),
                    data.get("context_id", "default"),
                    data.get("data_type", "generic"),
                ...
                logger.info(f"✅ Data stored in SQLite: {data_id}")
                return f"sqlite://a2a_data_records/{data_id}"
                
            except Exception as e:
                logger.error(f"SQLite storage failed: {e}, falling back to filesystem")
                return await self._store_data(data, "filesystem")
        else:
    ...
    async def _retrieve_data(self, data_id: str, storage_backend: str) -> Dict[str, Any]:
        ...
        if storage_backend == "filesystem":
            ...
            try:
                ...
                logger.error(f"HANA retrieval failed: {e}, trying SQLite")
                if "sqlite" in self.storage_clients:
                    return await self._retrieve_data(data_id, "sqlite")
                else:
                    raise
            ...
            try:
                cursor = self.sqlite_connection.cursor()
                cursor.execute('''
                    SELECT data, metadata FROM a2a_data_records 
                    WHERE record_id = ? AND is_deleted = FALSE
                ''', (data_id,))
                ...
                if result:
                    ...
                    raise ValueError(f"Data not found in SQLite: {data_id}")
            ...
            elif "sqlite" in self.storage_clients:
                return await self._retrieve_data(data_id, "sqlite")
            else:
    ...
    async def _delete_data(self, data_id: str, storage_backend: str):
        ...
        if storage_backend == "filesystem":
            ...
            try:
                ...
                cursor = self.sqlite_connection.cursor()
                cursor.execute('''
                    UPDATE a2a_data_records 
                    SET is_deleted = TRUE, updated_at = ? 
                    WHERE record_id = ?
                ''', (datetime.utcnow().isoformat(), data_id))
                self.sqlite_connection.commit()
                logger.info(f"Data soft-deleted in SQLite: {data_id}")
