...
from .aiDecisionLogger import (
    DecisionType, OutcomeStatus, AIDecision, DecisionOutcome, PatternInsight
)
...
class AIDecisionDatabaseLogger:
        ...
        try:
            outcome_data = {
                ...
                "data": {
                    ...
                    "outcome_timestamp": datetime.fromtimestamp(outcome.outcome_timestamp).isoformat(),
        ...
        try:
            ...
            query_data = {
                ...
                "query": {
                    ...
                    "where": {
                        "agent_id": self.agent_id,
                        "pattern_type": decision_type.value
                    },
        ...
        try:
            ...
            report = {
                ...
                "data_quality": {
                    ...
                    "decisions_with_outcomes": analytics.get("summary", {}).get("decisions_with_outcomes", 0),
    ...
    def _extract_confidence(self, ai_response: Dict[str, Any]) -> float:
        ...
        if isinstance(ai_response, dict):
            ...
            elif "advisor_response" in ai_response and isinstance(ai_response["advisor_response"], dict):
    ...
    def _identify_patterns(self, decisions: List[Dict[str, Any]]) -> List[PatternInsight]:
        ...
        for decision_type, type_decisions in by_type.items():
            if len(type_decisions) >= self.learning_threshold:
                success_rate = sum(1 for d in type_decisions if d.get("success_score", 0) > 0.5) / len(type_decisions)
                avg_confidence = sum(d.get("confidence_score", 0.5) for d in type_decisions) / len(type_decisions)
    ...
    def _process_database_analytics(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for row in data:
            ...
            if row.get("total_decisions", 0) > 0:
                success_rates[decision_type] = row.get("successful_outcomes", 0) / row.get("total_decisions", 1)
    ...
    def _get_cached_recommendations(self, decision_type: DecisionType, context: Dict[str, Any]) -> List[str]:
