...
from .aiDecisionLoggerDatabase import (
    AIDecisionDatabaseLogger,
    get_global_database_decision_registry,
)
...
class AIDatabaseDecisionIntegrationMixin:
    ...
    async def _enhanced_handle_advisor_request(
        self, message: A2AMessage, context_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, "ai_advisor") and self.ai_advisor:
                ...
                advisor_response = await self.ai_advisor.process_a2a_help_message(
                    [part.dict() for part in message.parts], context.get("source_agent_id")
                )
                ...
                has_answer = (
                    isinstance(advisor_response, dict)
                    and advisor_response.get("answer")
                    and len(advisor_response["answer"]) > 10
                )

                confidence = (
                    advisor_response.get("confidence", 0.5)
                    if isinstance(advisor_response, dict)
                    else 0.5
                )
                ...
                await self.ai_decision_logger.log_outcome(
                    ...
                    outcome_status=(
                        OutcomeStatus.SUCCESS if has_answer else OutcomeStatus.PARTIAL_SUCCESS
                    ),
                    success_metrics={
                ...
                recommendations = await self.ai_decision_logger.get_recommendations(
                    DecisionType.ADVISOR_GUIDANCE, context
                )
                ...
                return {
                    ...
                    "agent_id": self.agent_id,
                    "contextId": context_id,
                    "decision_metadata": {"decision_id": decision_id, "database_backed": True},
                }
    ...
    async def _enhanced_handle_error_with_help_seeking(
        self, error: Exception, operation_name: str, context_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, "seek_help_for_error"):
                ...
                await self.ai_decision_logger.log_outcome(
                    decision_id=decision_id,
                    outcome_status=(
                        OutcomeStatus.SUCCESS if help_received else OutcomeStatus.FAILURE
                    ),
                    success_metrics={
    ...
    async def _log_task_planning_decision(
        self, task_description: str, plan_result: Dict[str, Any]
    ) -> str:
    ...
    async def _update_task_outcome(
        self, decision_id: str, task_successful: bool, task_duration: float = 0.0
    ):
    ...
    async def _log_delegation_decision(
        self, delegate_agent: str, actions: list, result: Dict[str, Any]
    ) -> str:
        ...
        await self.ai_decision_logger.log_outcome(
            decision_id=decision_id,
            outcome_status=(
                OutcomeStatus.SUCCESS if delegation_successful else OutcomeStatus.FAILURE
            ),
            success_metrics={
    ...
    async def _log_quality_assessment_decision(
        self, data_quality: Dict[str, Any], assessment_result: Dict[str, Any]
    ) -> str:
        ...
        await self.ai_decision_logger.log_outcome(
            decision_id=decision_id,
            outcome_status=(
                OutcomeStatus.SUCCESS if quality_passed else OutcomeStatus.PARTIAL_SUCCESS
            ),
            success_metrics={
    ...
    async def query_decision_patterns(
        self, pattern_filters: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # httpx\.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"AI Decision schema initialization: {'SUCCESS' if success else 'FAILED'}"
                )
                return success
            else:
                logger.error(
                    f"Schema initialization failed: {response.status_code} - {response.text}"
                )
                return False
...
async def migrate_json_data_to_database(
    json_storage_path: str, data_manager_url: str, agent_id: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        db_logger = AIDecisionDatabaseLogger(agent_id=agent_id, data_manager_url=data_manager_url)
