...
class AgentIdentity:
    ...
    def __init__(self, agent_id: str, agent_type: str, public_key: bytes, private_key: bytes = None):
        ...
        self.trust_score = 0.0  # New agents start with zero trust
        self.verified = False
        self.failed_verifications = 0  # Track failed verification attempts
        self.locked_until = None  # Agent lockout timestamp
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'AgentIdentity':
        ...
        identity.trust_score = data.get("trust_score", 0.0)
        identity.verified = data.get("verified", False)
        identity.failed_verifications = data.get("failed_verifications", 0)
        identity.locked_until = datetime.fromisoformat(data["locked_until"]) if data.get("locked_until") else None
        return identity
...
class SmartContractTrust:
    ...
    def __init__(self, contract_id: str = None):
        ...
        # Contract rules - more secure defaults
        self.trust_rules = {
            "min_trust_score": 0.8,  # Require higher trust for message acceptance
            "message_expiry_minutes": 10,  # Shorter expiry for better security
            "max_trust_degradation": 0.2,  # Faster trust degradation for bad actors
            "trust_recovery_rate": 0.02,  # Slower trust recovery to prevent gaming
            "signature_required": True,
            "require_verification": True,  # Require agent verification before high trust
            "min_key_size": 2048,  # Minimum RSA key size
            "max_failed_verifications": 3  # Lock agent after failures
        }
    ...
    def register_agent(
        ...
        try:
            # Generate RSA key pair if not provided - use secure key size
            if not public_key or not private_key:
                key_size = max(self.trust_rules["min_key_size"], 3072)  # Use at least 3072 bits for future security
            ...
            # Initialize trust relationships with other agents - start with zero trust
            for other_agent_id in self.agents:
                if other_agent_id != agent_id:
                    # Start with zero trust - trust must be earned through successful interactions
                    self.trust_relationships[agent_id][other_agent_id] = 0.0
                    self.trust_relationships[other_agent_id][agent_id] = 0.0
    ...
    def verify_message(self, signed_message: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            # Check if agent is currently locked due to failed verifications
            if identity.locked_until and datetime.utcnow() < identity.locked_until:
                remaining = (identity.locked_until - datetime.utcnow()).total_seconds() / 60
                return False, {"error": f"Agent locked for {remaining:.1f} more minutes due to failed verifications"}
            ...
            try:
                ...
                # Check trust score and verification status
                trust_score = identity.trust_score
                if trust_score < self.trust_rules["min_trust_score"]:
                    return False, {"error": f"Agent trust score too low: {trust_score}"}
                
                # Require agent verification for high trust operations
                if self.trust_rules["require_verification"] and not identity.verified and trust_score > 0.5:
                    return False, {"error": "Agent verification required for this trust level"}
    ...
    def establish_trust_channel(self, agent1_id: str, agent2_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate cryptographically secure shared secret for the channel
            import secrets
            shared_secret = secrets.token_bytes(32)  # Use secrets module for cryptographic randomness
        ...
        try:
            ...
            if agent_id in self.agents:
                ...
                if success:
                    ...
                    # Reset failed verification count on success
                    identity.failed_verifications = 0
                    ...
                    identity.failed_verifications += 1
                    
                    # Lock agent if too many failed verifications
                    if identity.failed_verifications >= self.trust_rules["max_failed_verifications"]:
                        identity.locked_until = datetime.utcnow() + timedelta(minutes=30)  # 30-minute lockout
                        logger.critical(f"🔒 Agent {agent_id} locked due to {identity.failed_verifications} failed verifications")
                    else:
                        logger.warning(f"⚠️ Trust score decreased for agent {agent_id}: {identity.trust_score} ({identity.failed_verifications} failures)")
    ...
    def verify_agent(self, agent_id: str, verification_proof: Dict[str, Any]) -> bool:
        """
        Verify an agent's identity and grant verified status
        
        Args:
            agent_id: Agent to verify
            verification_proof: Proof of agent legitimacy (e.g., signed certificate)
        ...
        try:
            ...
            required_fields = ["agent_certificate", "issuer", "signature"]
            if not all(field in verification_proof for field in required_fields):
                logger.warning(f"❌ Incomplete verification proof for agent {agent_id}")
                return False
            
            # In a real implementation, this would verify against a certificate authority
            # For now, we'll do basic validation
            
            # Verify the agent certificate signature
            cert_data = verification_proof["agent_certificate"]
            expected_agent_id = cert_data.get("agent_id")
            
            if expected_agent_id != agent_id:
                logger.warning(f"❌ Agent ID mismatch in verification proof: {expected_agent_id} != {agent_id}")
                return False
            
            # Mark agent as verified
            identity.verified = True
            identity.trust_score = max(identity.trust_score, 0.3)  # Boost trust score for verified agents
