...
class ProjectService extends cds.Service {
     ...
     */
    @invalidateCache(['projects:all:*'], ['project'])
    ...
    @invalidateCache(
        [
            function(req) { return `project:${req.params[0]?.ID}`; },
            'projects:all:*'
        ],
        ['project']
    )
     ...
     */
    @cache('agent', 300, function(projectId) {
        return `agents:project:${projectId}`;
    })
     ...
     */
    @cache('workflow', 300, function(projectId) {
        return `workflows:project:${projectId}`;
    })
     ...
     */
    @cache('metrics', 60, function(projectId) {
