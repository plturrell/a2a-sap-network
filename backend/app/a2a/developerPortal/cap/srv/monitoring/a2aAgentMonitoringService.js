...
class A2AAgentMonitoringService {
     ...
     */
    async _getMonitoringData(req, res) {
        try {
    ...
    async _getAgentsList() {
        ...
        const agentsWithMetrics = await Promise.all(
            agentDefinitions.map(async (agentDef) => {
    ...
    _determineAgentStatus(metrics) {
        if (!metrics.timestamp) return 'unknown';
        
        const lastSeen = new Date(metrics.timestamp);
        const now = new Date();
        const timeDiff = now - lastSeen;
        
        if (timeDiff > 300000) return 'down'; // 5 minutes
        if (timeDiff > 60000) return 'degraded'; // 1 minute
        return 'running';
    }

    _calculateHealthScore(metrics) {
        if (!metrics.system_metrics) return 50;
    ...
    async _getAgentDetails(req, res) {
        try {
    ...
    async _getAgentHealth(req, res) { /* Implementation */ }
    async _getAgentMetrics(req, res) { /* Implementation */ }
    async _getAgentAlerts(req, res) { /* Implementation */ }
    async _getPerformanceSummary(req, res) { /* Implementation */ }
    async _getPerformanceTrends(req, res) { /* Implementation */ }
    async _getAgentComparison(req, res) { /* Implementation */ }
    async _getAllAlerts(req, res) { /* Implementation */ }
    async _acknowledgeAlert(req, res) { /* Implementation */ }
    async _dismissAlert(req, res) { /* Implementation */ }
    async _setupWebSocketMetrics(req, res) { /* Implementation */ }
    async _exportMetricsCSV(req, res) { /* Implementation */ }
    async _exportMetricsExcel(req, res) { /* Implementation */ }
    async _exportMetricsPDF(req, res) { /* Implementation */ }
}
