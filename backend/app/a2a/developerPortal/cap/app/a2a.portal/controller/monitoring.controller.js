sap.ui.define([
    ...
    return Controller.extend("a2a.portal.controller.Monitoring", {
        ...
        onLogLevelFilter: function (oEvent) {
            var sSelectedLevel = oEvent.getParameter("selectedItem").getKey();
            var oTable = this.byId("logsTable");
            
            if (oTable) {
                var oBinding = oTable.getBinding("items");
                if (sSelectedLevel !== "all") {
                    var oFilter = new sap.ui.model.Filter("level", sap.ui.model.FilterOperator.EQ, sSelectedLevel.toUpperCase());
                    oBinding.filter([oFilter]);
                } else {
                    oBinding.filter([]);
                }
