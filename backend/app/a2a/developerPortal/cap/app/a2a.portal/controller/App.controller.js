...
], function (Controller, MessageToast, MessageBox, Fragment, JSONModel, ODataModel, Log, BusyIndicator, encodeXML) {
    "use strict";
    ...
    return Controller.extend("a2a.portal.controller.App", {
        ...
        _handleNotificationSuccess: function (oResponse, oData, oModel) {
            try {
                ...
                oData.stats.unread = oData.notifications.filter(function(n) { return n.status === 'unread'; }).length;
        ...
        _handleNotificationError: function (jqXHR, iRetryCount, oData, oModel) {
            var sErrorMessage = "Failed to load notifications";
            
            oData.loading = false;
            
            // Implement exponential backoff retry logic
            if (iRetryCount < 3 && (jqXHR.status === 0 || jqXHR.status >= 500)) {
                var iDelay = Math.pow(2, iRetryCount) * 1000; // 1s, 2s, 4s
        ...
        _initializeSettingsModel: function () {
            ...
            this._loadUserSettings().then(function(oSettings) {
        ...
        _resetToDefaults: function () {
            this._loadUserSettings().then(function(oDefaultSettings) {
                var oSettingsModel = this._oSettingsDialog.getModel("settings");
        ...
        _showApplicationLogs: function () {
            ...
            jQuery.ajax({
                ...
                success: function (oResponse) {
                    ...
                    var sLogs = that._formatApplicationLogs(oResponse.logs || []);
        ...
        _formatApplicationLogs: function (aLogs) {
            var sFormattedLogs = "=== SAP A2A Developer Portal - Application Logs ===\n\n";
            ...
            aLogs.forEach(function (oLogEntry) {
                var sTimestamp = new Date(oLogEntry.timestamp).toISOString();
                var sLevel = (oLogEntry.level || "INFO").toUpperCase();
                var sMessage = encodeXML(oLogEntry.message || "No message");
                var sComponent = oLogEntry.component || "unknown";
        ...
        _getBearerToken: function () {
            ...
            var oSecurityContext = this.getView().getModel("securityContext");
        ...
        _logSecurityEvent: function (sEventType, mDetails) {
            ...
            // Add custom details
            if (mDetails) {
                Object.keys(mDetails).forEach(function(sKey) {
        ...
        onNavigate: function (oEvent) {
            ...
            var sFromPage = oEvent.getParameter("from");
            var sToPage = oEvent.getParameter("to");
        ...
        _initializeRoleBasedFeatures: function (aScopes) {
            var oFeatureModel = new JSONModel({
        ...
        _validatePageAccess: function (sPageId) {
            var oFeatures = this.getView().getModel("features");
            if (!oFeatures) {
                return false;
            }
            
            var oFeatureData = oFeatures.getData();
            var bHasAccess = true;
        ...
        _processNotifications: function (aNotifications) {
            return aNotifications.map(function (oNotification) {
                return {
                    id: oNotification.id,
        ...
        _updateBrowserTitle: function (iUnreadCount) {
            var sTitle = this._oResourceBundle.getText("app.title");
        ...
        _checkForCriticalNotifications: function (aNotifications) {
            var aCriticalNotifications = aNotifications.filter(function (oNotification) {
        ...
        _showDesktopNotification: function (oNotification) {
            if (this._supportsDesktopNotifications()) {
                var oDesktopNotification = new Notification(oNotification.title, {
