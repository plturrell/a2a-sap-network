    ...
    return BaseController.extend("sap.a2a.controller.ProjectsListReport", {
        ...
        _performAggregation: function (aContexts, oAggregation) {
            ...
            aContexts.forEach(oContext => {
                // Skip group headers
                if (oContext.isGroupHeader && oContext.isGroupHeader()) return;
...
<body>`;
    ...
    <div class="footer">`;
        ...
        _generatePrintTableHTML: function (oSettings) {
            ...
            // Add headers
            if (oSettings.columns.name !== false) sHTML += "<th>Project Name</th>";
            if (oSettings.columns.status) sHTML += "<th>Status</th>";
            if (oSettings.columns.priority) sHTML += "<th>Priority</th>";
            if (oSettings.columns.agents) sHTML += "<th>Agents</th>";
            if (oSettings.columns.lastDeployment) sHTML += "<th>Last Deployment</th>";
            if (oSettings.columns.createdBy) sHTML += "<th>Created By</th>";
        ...
        _generatePDF: function (oSettings) {
            ...
            sap.ui.require(["sap/ui/thirdparty/jsPDF"], function (jsPDF) {
                // Create PDF document
                const doc = new jsPDF({
         ...
         * Add KPI section
         */
        _addKPISection: function (doc, oSettings) {
            const oViewModel = this.getModel("listReport");
            const oKPIs = oViewModel.getProperty("/kpi");
         ...
         */
        _addTableData: function (doc, oSettings) {
            const aData = this._getExportData("visible");
            const aColumns = this._getExportColumns();
...
<body>`;
    ...
    <div class="toc">
        ...
        <ol>`;
                if (oSettings.includeKPIs) sHTML += `<li>Key Performance Indicators</li>`;
                if (oSettings.includeFilters) sHTML += `<li>Active Filters</li>`;
                if (oSettings.includeTable) sHTML += `<li>Project Data</li>`;
                sHTML += `
        </ol>
    </div>`;
            }
