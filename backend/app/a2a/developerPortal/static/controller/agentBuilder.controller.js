...
sap.ui.define([
    ...
    return Controller.extend("a2a.portal.controller.AgentBuilder", {
        ...
        _hasCustomChanges: function () {
            var oAgentModel = this.getView().getModel("agent");
            var aSkills = oAgentModel.getProperty("/skills") || [];
            var aHandlers = oAgentModel.getProperty("/handlers") || [];
            var sName = oAgentModel.getProperty("/name");
            var sDescription = oAgentModel.getProperty("/description");
        ...
        onAddSelectedCapabilities: function () {
            ...
            let aCurrentSkills = oAgentModel.getProperty("/skills") || [];
...
agent.run();`;
        ...
        onToggleDebugMode: function () {
            var oModel = this.getView().getModel();
            var bCurrentDebugMode = oModel.getProperty("/debugMode");
            var bNewDebugMode = !bCurrentDebugMode;
        ...
        onSetBreakpoint: function () {
            ...
            var aBreakpoints = oModel.getProperty("/debugBreakpoints") || [];
            var sBreakpointId = "bp_" + Date.now();
            aBreakpoints.push({
                id: sBreakpointId,
         ...
         */
        onCheckUniqueness: function (oEvent) {
            const oAgentModel = this.getView().getModel("agent");
        ...
        onUploadIcon: function () {
            ...
            oFileInput.addEventListener("change", function (oEvent) {
                ...
                // Validate file type
                var aAllowedTypes = ["image/png", "image/jpg", "image/jpeg", "image/svg+xml"];
        ...
        onGenerateAIIcon: function () {
            ...
            var sAgentType = oAgentModel.getProperty("/type");
            var sDescription = oAgentModel.getProperty("/description");
            ...
            var sBusyDialogText = "Generating AI icon for '" + sAgentName + "'...";
        ...
        _generateIconFromPrompt: function (sName, sType, sDescription) {
            var sPrompt = "Generate a professional icon for an AI agent named '" + sName + 
                         "' of type '" + sType + "'. Description: " + (sDescription || "No description");
            
            // In real implementation, call AI image generation service
            // For now, use a placeholder
            var sGeneratedIconUrl = "data:image/svg+xml;base64," + btoa(`
        ...
        onSelectDefaultIcon: function (oEvent) {
            ...
            var sIconSrc = oSource.data("icon");
            var sIconName = oSource.getItems()[1].getText(); // Get the text from the second item (Text control)
        ...
        onUpdateTags: function (oEvent) {
            ...
            var aTags = aTokens.map(function (oToken) {
                return {
                    key: oToken.getKey(),
                    text: oToken.getText()
                };
            ...
            var sTagsString = aTags.map(function (oTag) {
                return oTag.text;
            }).join(", ");
        ...
        onDeleteTag: function (oEvent) {
            ...
            var oAgentModel = this.getView().getModel("agent");
            var aTags = oAgentModel.getProperty("/metadata/tags");
            
            // Remove tag from array
            var aFilteredTags = aTags.filter(function (oTag) {
            ...
            // Update tags string
            var sTagsString = aFilteredTags.map(function (oTag) {
                return oTag.text;
            }).join(", ");
        ...
        _autoFillMetadata: function () {
            ...
            if (oAgent.template && !oAgent.metadata.category) {
                ...
                var sSuggestedCategory = sCategoryMapping[oAgent.template];
            ...
            if (!oAgent.metadata.tags.length) {
                ...
                if (aSuggestedTags.length) {
                    ...
                    var sTagsString = aSuggestedTags.map(function (oTag) {
                        return oTag.text;
                    }).join(", ");
                    oAgentModel.setProperty("/metadata/tagsString", sTagsString);
                }
        ...
        onBuildAgent: function () {
            ...
            var oDesignTabBar = this.byId("designTabBar");
            if (oDesignTabBar) {
        ...
        _executeBuildStep: function (stepNumber, stepName, stepFunction) {
            ...
            return new Promise(function(resolve, reject) {
                // Update progress
                var progress = Math.round((stepNumber - 1) / 6 * 100);
                ...
                setTimeout(function() {
                    try {
                        ...
                        if (result && result.warnings) {
                            var currentWarnings = oViewModel.getProperty("/buildWarnings");
                            oViewModel.setProperty("/buildWarnings", currentWarnings + result.warnings);
                        }
                        resolve();
                    } catch (error) {
                        var currentErrors = oViewModel.getProperty("/buildErrors");
        ...
        _generateBuildCode: function () {
            var oAgentModel = this.getView().getModel("agent");
            var oViewModel = this.getView().getModel();
            var sLanguage = oViewModel.getProperty("/codeLanguage");
        ...
        _resolveDependencies: function () {
            ...
            var dependencies = ["a2a-sdk>=2.0.0"];
        ...
        _compileAgent: function () {
            var oViewModel = this.getView().getModel();
            var sCode = oViewModel.getProperty("/generatedCode");
        ...
        _appendBuildOutput: function (sText) {
            var oViewModel = this.getView().getModel();
            var currentOutput = oViewModel.getProperty("/buildOutput");
        ...
        onDownloadArtifacts: function () {
            ...
            var zipContent = "# Generated Agent Package\n" +
                           "# Agent: " + oAgentData.name + "\n" +
                           "# ID: " + oAgentData.id + "\n" +
                           "# Generated: " + new Date().toISOString() + "\n";
            
            var blob = new Blob([zipContent], { type: "text/plain" });
        ...
        onViewArtifact: function (oEvent) {
            ...
            var oArtifact = oBindingContext.getObject();
