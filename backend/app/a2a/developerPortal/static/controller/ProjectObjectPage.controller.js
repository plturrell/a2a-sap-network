sap.ui.define([
    ...
    return BaseController.extend("a2a.portal.controller.ProjectObjectPage", {
        ...
        _loadProjectDetails: function (sProjectId) {
            ...
            jQuery.ajax({
                ...
                error: function () {
                    ...
                    var oMockData = this._getMockProjectData(sProjectId);
        ...
        _enrichProjectData: function (oData) {
            ...
            if (oData.activities) {
                oData.activities.forEach(function (activity) {
        ...
        _calculateProgress: function (oProject) {
            if (!oProject.startDate || !oProject.endDate) return 0;
            
            var start = new Date(oProject.startDate).getTime();
            var end = new Date(oProject.endDate).getTime();
            var now = Date.now();
        ...
        onEdit: function () {
            var bEditMode = this.getModel("view").getProperty("/editMode");
        ...
        _validateEditForm: function () {
            var oProjectData = this.getModel("project").getData();
            var oValidationResult = this._performComprehensiveValidation(oProjectData);
        ...
        _performComprehensiveValidation: function (oProjectData) {
            ...
            var aFieldsToValidate = ["name", "description", "startDate", "endDate", "budget", "costCenter"];
            oResult.summary.totalFields = aFieldsToValidate.length;
            
            // Validate each field individually
            aFieldsToValidate.forEach(function(sField) {
                var oFieldResult = this._validateField(sField, oProjectData[sField], oProjectData);
            ...
            var oCrossFieldResult = this._validateCrossFieldRules(oProjectData);
        ...
        _validateProjectName: function (sName) {
            ...
            if (!sName || sName.trim() === "") {
                ...
                var aReservedWords = ["admin", "system", "test", "demo"];
        ...
        _validateStartDate: function (dStartDate, oProjectData) {
            ...
            if (dStartDate) {
                var startDate = new Date(dStartDate);
                var now = new Date();
        ...
        _validateCrossFieldRules: function (oProjectData) {
            ...
            if (oProjectData.startDate && oProjectData.endDate && oProjectData.budget) {
                var duration = (new Date(oProjectData.endDate) - new Date(oProjectData.startDate)) / (1000 * 60 * 60 * 24);
                var dailyBudget = oProjectData.budget / duration;
        ...
        _updateValidationDisplay: function (oValidationResult) {
            ...
            Object.keys(oValidationResult.fieldValidation).forEach(function(sField) {
                var oFieldResult = oValidationResult.fieldValidation[sField];
        ...
        _updateFieldValidationDisplay: function (sField, oFieldResult) {
            var sControlId = this._getFieldControlId(sField);
        ...
        _showValidationSummary: function (oValidationResult) {
            var sMessage = "Form validation found issues:\n\n";
            
            if (oValidationResult.summary.totalErrors > 0) {
                sMessage += "Errors (" + oValidationResult.summary.totalErrors + "):\n";
                oValidationResult.errors.forEach(function(sError, index) {
                    sMessage += "• " + sError + "\n";
                });
                sMessage += "\n";
            }
            ...
            if (oValidationResult.summary.totalWarnings > 0) {
                ...
                oValidationResult.warnings.forEach(function(sWarning, index) {
                    sMessage += "• " + sWarning + "\n";
                });
            }
        ...
        _showDetailedValidationDialog: function (oValidationResult) {
            ...
            var oModel = new sap.ui.model.json.JSONModel(oValidationResult);
        ...
        onRealTimeValidation: function (oEvent) {
            ...
            var sFieldName = this._getFieldNameFromControl(oControl);
            
            if (sFieldName) {
                var oProjectData = this.getModel("project").getData();
                oProjectData[sFieldName] = sValue;
                
                var oFieldResult = this._validateField(sFieldName, sValue, oProjectData);
        ...
        _getFieldNameFromControl: function (oControl) {
            ...
            var sLocalId = sControlId.split("--").pop();
        ...
        onToggleValidationPanel: function () {
            var bShow = this.getModel("view").getProperty("/showValidationPanel");
        ...
        _toggleFormControls: function (bEditable) {
            ...
            aEditableControls.forEach(function (sControlId) {
                var oControl = this.byId(sControlId);
        ...
        _updateEditModeStyles: function (bEditMode) {
            ...
            var oStyleElement = document.getElementById("editModeStyles");
        ...
        _addEditIndicator: function () {
            var $headerContent = $('.sapUxAPObjectPageHeaderContent').first();
            if ($headerContent.length && !$headerContent.find('.a2a-edit-indicator').length) {
                var $indicator = $('<div class="a2a-edit-indicator">EDIT MODE</div>');
        ...
        onCancelEdit: function () {
            ...
            var oCurrentData = this.getModel("project").getData();
            var oChanges = this._calculateChanges(this._originalData, oCurrentData);
        ...
        _showAdvancedCancelDialog: function (oChanges) {
            var sChangesSummary = this._formatChangesForCancelDialog(oChanges);
            var sDialogContent = "You have unsaved changes:\n\n" + sChangesSummary + "\n\nWhat would you like to do?";
        ...
        _formatChangesForCancelDialog: function (oChanges) {
            var aChangeDescriptions = [];
            Object.keys(oChanges).forEach(function (sField) {
                var oChange = oChanges[sField];
                var sFromValue = oChange.from ? String(oChange.from) : "(empty)";
                var sToValue = oChange.to ? String(oChange.to) : "(empty)";
                
                // Truncate long values for dialog display
                if (sFromValue.length > 50) sFromValue = sFromValue.substring(0, 47) + "...";
                if (sToValue.length > 50) sToValue = sToValue.substring(0, 47) + "...";
        ...
        _showChangeReviewDialog: function (oChanges) {
            var oModel = new sap.ui.model.json.JSONModel({
                changes: Object.keys(oChanges).map(function (sField) {
                    return {
                        field: sField,
        ...
        onSelectiveCancel: function () {
            var oChangesModel = this._changeReviewDialog.getModel("changes");
            var aChanges = oChangesModel.getProperty("/changes");
            var aSelectedFields = aChanges.filter(function (oChange) {
        ...
        _discardSelectedChanges: function (aFieldsToRevert) {
            var oCurrentData = this.getModel("project").getData();
            var oOriginalData = this._originalData;
            
            // Revert only selected fields
            aFieldsToRevert.forEach(function (sField) {
            ...
            var oRemainingChanges = this._calculateChanges(oOriginalData, oCurrentData);
        ...
        _saveAndExitEditMode: function () {
            ...
            var bSaveSuccessful = this._saveChanges();
        ...
        _clearChangeIndicators: function () {
            ...
            var aFormControls = ["projectNameInput", "projectDescInput", "startDatePicker", "endDatePicker", "budgetInput", "costCenterInput"];
            
            aFormControls.forEach(function (sControlId) {
                var oControl = this.byId(sControlId);
                if (oControl) {
        ...
        onAutoSaveDraft: function () {
            ...
            var oCurrentData = this.getModel("project").getData();
            var oDraftData = jQuery.extend(true, {}, oCurrentData);
        ...
        onRestoreFromDraft: function () {
            try {
                var sDraftData = localStorage.getItem("a2a_project_draft_" + this._projectId);
                if (sDraftData) {
                    var oDraftData = JSON.parse(sDraftData);
        ...
        _saveChanges: function () {
            ...
            var oChanges = this._calculateChanges(oOriginalData, oProjectData);
            if (Object.keys(oChanges).length === 0) {
            ...
            this._checkForConflicts(oProjectData).then(function(bConflictExists) {
                if (bConflictExists) {
        ...
        _calculateChanges: function (oOriginal, oCurrent) {
            var oChanges = {};
            var aTrackableFields = ["name", "description", "startDate", "endDate", "budget", "costCenter", "priority", "status"];
            
            aTrackableFields.forEach(function(sField) {
                var originalValue = oOriginal ? oOriginal[sField] : null;
            ...
            return new Promise(function(resolve) {
                ...
                jQuery.ajax({
                    ...
                    success: function (versionData) {
                        var bConflict = versionData.lastModified !== oProjectData.lastModified;
                        resolve(bConflict);
                    }.bind(this),
                    error: function () {
        ...
        _completeSaveSuccess: function (response, oChanges) {
            ...
            var sChangesSummary = this._formatChangesSummary(oChanges);
        ...
        _handleSaveError: function (xhr, status, error, oChanges) {
            ...
            var sErrorMessage = "Failed to save changes";
            var sErrorDetails = "";
            
            // Parse error response
            try {
                var oErrorResponse = JSON.parse(xhr.responseText);
        ...
        _logChangeHistory: function (oChanges) {
            ...
            var aChangeHistory = this.getModel("view").getProperty("/changeHistory") || [];
        ...
        onViewChangeHistory: function () {
            ...
            var sHistoryText = aChangeHistory.map(function(oChange, index) {
        ...
        _updateFieldEditability: function () {
            var oEditabilityConfig = this.getModel("view").getProperty("/fieldEditability");
            var sUserRole = oEditabilityConfig.userRole;
            var sProjectStatus = this.getModel("project").getProperty("/status") || "ACTIVE";
            
            // Get role-based permissions
            var aRolePermissions = oEditabilityConfig.rolePermissions[sUserRole] || [];
            
            // Get status-based restrictions
            var aStatusRestrictions = oEditabilityConfig.statusRestrictions[sProjectStatus];
            ...
            var aEditableFields = aRolePermissions.filter(function(field) {
            ...
            ["name", "description", "startDate", "endDate", "budget", "costCenter", "priority", "status"].forEach(function(field) {
        ...
        _applyFieldEditabilityToControls: function () {
            var oEditableFields = this.getModel("view").getProperty("/fieldEditability/editableFields");
            var bEditMode = this.getModel("view").getProperty("/editMode");
            
            // Apply editability to each field
            Object.keys(oEditableFields).forEach(function (sField) {
                var bFieldEditable = oEditableFields[sField];
                var sControlId = this._getFieldControlId(sField);
                var oControl = this.byId(sControlId);
        ...
        _attachReadOnlyTooltips: function () {
            var that = this;
            var aFieldIds = ["projectNameInput", "projectDescInput", "startDatePicker", "endDatePicker", "budgetInput", "costCenterInput"];
            
            aFieldIds.forEach(function (sControlId) {
                var oControl = that.byId(sControlId);
                if (oControl) {
                    oControl.attachBrowserEvent("mouseenter", function () {
                        var oEditableFields = that.getModel("view").getProperty("/fieldEditability/editableFields");
                        var sField = that._getFieldFromControlId(sControlId);
                        var bEditMode = that.getModel("view").getProperty("/editMode");
                        
                        if (bEditMode && !oEditableFields[sField]) {
                            var sReason = that._getReadOnlyReason(sField);
        ...
        _getReadOnlyReason: function (sField) {
            var oConfig = this.getModel("view").getProperty("/fieldEditability");
            var sUserRole = oConfig.userRole;
            var sProjectStatus = oConfig.projectStatus;
            
            var aRolePermissions = oConfig.rolePermissions[sUserRole] || [];
            var aStatusRestrictions = oConfig.statusRestrictions[sProjectStatus];
        ...
        _attachFieldFocusHandlers: function () {
            ...
            aFieldIds.forEach(function (sControlId) {
                ...
                if (oControl && oControl.attachBrowserEvent) {
                    oControl.attachBrowserEvent("focus", function () {
                        that._onFieldFocus(sControlId);
                    });
                    
                    oControl.attachBrowserEvent("blur", function () {
        ...
        _onFieldFocus: function (sControlId) {
            var oControl = this.byId(sControlId);
            var sField = this._getFieldFromControlId(sControlId);
            var oEditableFields = this.getModel("view").getProperty("/fieldEditability/editableFields");
            var bEditMode = this.getModel("view").getProperty("/editMode");
        ...
        onTestFieldEditability: function () {
            var oConfig = this.getModel("view").getProperty("/fieldEditability");
            var sMessage = "Current Role: " + oConfig.userRole + "\n";
            sMessage += "Project Status: " + oConfig.projectStatus + "\n";
            sMessage += "Editable Fields: " + Object.keys(oConfig.editableFields).filter(function(field) {
        ...
        onAgentPress: function (oEvent) {
            var oAgent = oEvent.getSource().getBindingContext("project").getObject();
        ...
        onWorkflowPress: function (oEvent) {
            var oWorkflow = oEvent.getSource().getBindingContext("project").getObject();
        ...
        onMemberPress: function (oEvent) {
            var oMember = oEvent.getSource().getBindingContext("project").getObject();
            MessageToast.show("Member details: " + oMember.user.displayName);
        },

        onManagerPress: function () {
            var oManager = this.getModel("project").getProperty("/projectManager");
        ...
        _onSectionNavigate: function (oEvent) {
            var sTargetId = oEvent.getParameter("targetId");
        ...
        _updateCurrentSection: function (sSectionId) {
            ...
            var aItems = this.getModel("view").getProperty("/anchorBarItems");
            var sCurrentSection = null;
        ...
        onAnchorBarPress: function (oEvent) {
            ...
            var sKey = oItem.getKey ? oItem.getKey() : oItem.getCustomData()[0].getValue();
        ...
        _smoothScrollToSection: function (oSection) {
            ...
            if ($section.length > 0) {
                var iTargetOffset = $section.offset().top - this._anchorBarConfig.scrollOffset;
                var $scrollContainer = oObjectPageLayout.$().find(".sapUxAPObjectPageScrollContainer");
        ...
        _navigateToPreviousSection: function () {
            var sCurrentSection = this.getModel("view").getProperty("/currentSection");
            var aItems = this.getModel("view").getProperty("/anchorBarItems");
            
            if (!sCurrentSection || !aItems) return;
        ...
        _focusAnchorBar: function () {
            var oObjectPageLayout = this.byId("objectPageLayout");
            if (oObjectPageLayout) {
                var oAnchorBar = oObjectPageLayout.getAnchorBar();
                if (oAnchorBar) {
                    oAnchorBar.focus();
                }
            }
        ...
        onToggleAnchorBar: function () {
            var bVisible = this.getModel("view").getProperty("/anchorBarVisible");
        ...
        _setupScrollSpy: function (oObjectPageLayout) {
            ...
            this._debouncedScrollHandler = this._debounce(function () {
        ...
        _setupScrollMemory: function (oObjectPageLayout) {
            ...
            var that = this;
            oObjectPageLayout.attachNavigate(function (oEvent) {
                var sFromSectionId = oEvent.getParameter("from");
                if (sFromSectionId) {
        ...
        _setupScrollDirectionTracking: function (oObjectPageLayout) {
            ...
            this._scrollDirectionHandler = function (oEvent) {
                var iCurrentScrollTop = that._getScrollTop();
        ...
        _handleScrollSpyUpdate: function () {
            if (!this._anchorBarConfig.enableScrollSpy) return;
            
            var oObjectPageLayout = this.byId("objectPageLayout");
            var aSections = oObjectPageLayout.getSections();
            var iScrollTop = this._getScrollTop();
            var sCurrentSection = null;
            ...
            for (var i = 0; i < aSections.length; i++) {
                ...
                if ($section.length > 0) {
                    var oSectionPos = $section.offset();
                    var iSectionTop = oSectionPos.top - this._anchorBarConfig.scrollOffset;
                    var iSectionBottom = iSectionTop + $section.outerHeight();
        ...
        _saveScrollPosition: function (sSectionId) {
            if (!this._anchorBarConfig.enableScrollMemory) return;
        ...
        _restoreScrollPosition: function (sSectionId) {
            ...
            var iSavedPosition = this._sectionScrollPositions[sSectionId];
        ...
        onScrollToBottom: function () {
            ...
            if ($scrollContainer.length > 0) {
                var iMaxScroll = $scrollContainer[0].scrollHeight - $scrollContainer.height();
        ...
        onScrollByPage: function (bDown) {
            ...
            if ($scrollContainer.length > 0) {
                var iCurrentScroll = $scrollContainer.scrollTop();
                var iPageSize = $scrollContainer.height() * 0.8; // 80% of viewport
                var iNewPosition = bDown ? 
        ...
        _handleIntersectionChanges: function (entries) {
            var oMostVisibleSection = null;
            var fMaxVisibility = 0;
            ...
            if (oMostVisibleSection) {
                var sSectionId = oMostVisibleSection.id;
        ...
        _highlightSection: function (sSectionId, bAnimate = true) {
            if (!this._highlightConfig.enabled) return;
        ...
        _highlightSectionTitle: function (oSection) {
            var $sectionTitle = oSection.$().find('.sapUxAPObjectPageSectionTitle');
        ...
        _removeAllSectionHighlights: function () {
            ...
            aSections.forEach(function (oSection) {
                ...
                if ($section.length > 0) {
                    ...
                    var $sectionTitle = $section.find('.sapUxAPObjectPageSectionTitle');
        ...
        onToggleSectionHighlighting: function () {
            var bEnabled = this.getModel("view").getProperty("/sectionHighlighting/enabled");
        ...
        onHighlightSection: function (oEvent) {
            ...
            var sSectionKey = oEvent.getParameter("sectionKey") || oEvent.getSource().data("sectionKey");
            if (sSectionKey) {
                var oObjectPageLayout = this.byId("objectPageLayout");
                var aSections = oObjectPageLayout.getSections();
        ...
        _enhanceAnchorBarHighlighting: function () {
            ...
            var oAnchorBar = oObjectPageLayout.getAnchorBar();
            if (oAnchorBar) {
                var aButtons = oAnchorBar.getContent();
        ...
        _addCollapseControlToSection: function (oSection) {
            ...
            var $header = $section.find('.sapUxAPObjectPageSectionHeader');
            var $title = $header.find('.sapUxAPObjectPageSectionTitle');
            
            if ($title.length > 0 && !$title.hasClass('a2a-collapse-enhanced')) {
                // Create collapse toggle container
                var $titleContainer = $('<div class="a2a-section-title-container"></div>');
                var $collapseIcon = $('<span class="a2a-collapse-icon sap-icon sap-icon--navigation-down-arrow"></span>');
        ...
        _addSectionBadge: function (oSection) {
            ...
            var iBadgeCount = this._getSectionBadgeCount(sSectionId);
            
            if (iBadgeCount > 0) {
                var $header = oSection.$().find('.sapUxAPObjectPageSectionHeader');
                var $titleContainer = $header.find('.a2a-section-title-container');
                
                if ($titleContainer.length > 0 && !$titleContainer.find('.a2a-section-badge').length) {
                    var $badge = $('<span class="a2a-section-badge">' + iBadgeCount + '</span>');
        ...
        _toggleSectionCollapse: function (sSectionId) {
            var oObjectPageLayout = this.byId("objectPageLayout");
            var oSection = oObjectPageLayout.getSections().find(function (s) {
            ...
            var $section = oSection.$();
            var bIsCollapsed = $section.hasClass('a2a-section-collapsed');
        ...
        _collapseSection: function (sSectionId) {
            ...
            var $section = oSection.$();
            var $content = $section.find('.sapUxAPObjectPageSectionContent');
            var $icon = $section.find('.a2a-collapse-icon');
            var $header = $section.find('.sapUxAPObjectPageSectionHeader');
        ...
        _expandSection: function (sSectionId) {
            ...
            var iNaturalHeight = $content.height();
            $content.css('max-height', '0px');
        ...
        _updateCollapsedSections: function (sSectionId, bCollapsed) {
            var aCollapsed = this.getModel("view").getProperty("/sectionCollapse/collapsedSections");
            var iIndex = aCollapsed.indexOf(sSectionId);
        ...
        _saveCollapsedState: function () {
            var aCollapsed = this.getModel("view").getProperty("/sectionCollapse/collapsedSections");
            var sProjectId = this._projectId || "default";
        ...
        _restoreCollapsedState: function () {
            if (!this._collapseConfig.persistState) return;
            
            var sProjectId = this._projectId || "default";
            
            try {
                var sStoredState = localStorage.getItem("a2a-collapsed-sections-" + sProjectId);
                if (sStoredState) {
                    var aCollapsed = JSON.parse(sStoredState);
                    this.getModel("view").setProperty("/sectionCollapse/collapsedSections", aCollapsed);
                    
                    // Apply collapsed state to sections
                    aCollapsed.forEach(function (sSectionId) {
                        this._collapseSection(sSectionId);
                    }.bind(this));
                }
            } catch (e) {
        ...
        onToggleCollapseMode: function () {
            var bEnabled = this.getModel("view").getProperty("/sectionCollapse/enabled");
        ...
        _isSectionVisible: function (oSection) {
            ...
            var oRect = $section[0].getBoundingClientRect();
            var iViewportHeight = window.innerHeight || document.documentElement.clientHeight;
        ...
        _createLazyPlaceholder: function (oSection) {
            ...
            if ($content.length > 0) {
                ...
                var sPlaceholderHtml = this._createPlaceholderHtml(sSectionId);
        ...
        _createPlaceholderHtml: function (sSectionId) {
            var sPlaceholderType = this._getPlaceholderType(sSectionId);
            var sTitle = this._getSectionDisplayName(sSectionId);
        ...
        _getPlaceholderType: function (sSectionId) {
            if (sSectionId.indexOf("agents") !== -1) return "table";
            if (sSectionId.indexOf("workflows") !== -1) return "grid";
            if (sSectionId.indexOf("team") !== -1) return "list";
            if (sSectionId.indexOf("metrics") !== -1) return "charts";
            if (sSectionId.indexOf("activity") !== -1) return "timeline";
            return "content";
        },
        
        _getSectionDisplayName: function (sSectionId) {
            if (sSectionId.indexOf("general") !== -1) return "General Information";
            if (sSectionId.indexOf("agents") !== -1) return "Agents";
            if (sSectionId.indexOf("workflows") !== -1) return "Workflows";
            if (sSectionId.indexOf("team") !== -1) return "Team Members";
            if (sSectionId.indexOf("metrics") !== -1) return "Metrics & Analytics";
            if (sSectionId.indexOf("activity") !== -1) return "Recent Activity";
                ...
                this._lazyObserver = new IntersectionObserver(function (entries) {
                    entries.forEach(function (entry) {
                        if (entry.isIntersecting) {
                            var sSectionId = entry.target.id;
        ...
        _loadSectionContent: function (sSectionId, bAnimate) {
            ...
            var aLoaded = this.getModel("view").getProperty("/lazyLoading/loadedSections");
        ...
        _fetchSectionData: function (sSectionId) {
            ...
            var iDelay = this._getSectionLoadTime(sSectionId);
            ...
            return new Promise(function (resolve, reject) {
                setTimeout(function () {
                    ...
                    var oData = this._generateSectionData(sSectionId);
                    resolve(oData);
                }.bind(this), iDelay);
            }.bind(this));
        },
        ...
        _getSectionLoadTime: function (sSectionId) {
            ...
            if (sSectionId.indexOf("metrics") !== -1) return 1500; // Metrics take longer
            if (sSectionId.indexOf("activity") !== -1) return 1200; // Activity timeline
            if (sSectionId.indexOf("agents") !== -1) return 800;    // Agent table
            if (sSectionId.indexOf("workflows") !== -1) return 600; // Workflow grid
            if (sSectionId.indexOf("team") !== -1) return 400;      // Team list
        ...
        _renderSectionContent: function (sSectionId, oData, bAnimate) {
            ...
            if ($content.length > 0) {
                ...
                var sOriginalContent = this._sectionContentCache[sSectionId] || this._generateSectionHTML(sSectionId, oData);
        ...
        _renderSectionError: function (sSectionId, oError) {
            var oSection = this.byId("objectPageLayout").getSections().find(function (s) {
                return s.getId() === sSectionId;
            });
            
            if (!oSection) return;
            
            var $content = oSection.$().find('.sapUxAPObjectPageSectionContent');
            var sErrorHtml = `
        ...
        _retrySectionLoad: function (sSectionId) {
            ...
            var aLoaded = this.getModel("view").getProperty("/lazyLoading/loadedSections");
            var iIndex = aLoaded.indexOf(sSectionId);
            if (iIndex !== -1) {
        ...
        _setSectionLoadingState: function (sSectionId, sState) {
            ...
            var oLoadingStates = this.getModel("view").getProperty("/lazyLoading/loadingStates") || {};
        ...
        onToggleLazyLoading: function () {
            var bEnabled = this.getModel("view").getProperty("/lazyLoading/enabled");
        ...
        _setupSuggestionTriggers: function () {
            ...
            var aFieldIds = ["projectNameInput", "projectDescInput", "costCenterInput", "budgetInput"];
        ...
        _onFieldInputChange: function (oEvent) {
            ...
            this._suggestionsEngine.activeTimeout = setTimeout(function () {
                this._generateContextualSuggestions(sFieldName, sValue);
            }.bind(this), this.getModel("view").getProperty("/suggestions/showDelay"));
        },
        
        _generateContextualSuggestions: function (sFieldName, sCurrentValue) {
            if (!this.getModel("view").getProperty("/suggestions/enabled")) return;
            if (!this.getModel("view").getProperty("/editMode")) return;
            ...
            var iMaxSuggestions = this.getModel("view").getProperty("/suggestions/maxSuggestions");
        ...
        _filterRejectedSuggestions: function (aSuggestions, sFieldName) {
            var oRejectionPatterns = this.getModel("view").getProperty("/suggestions/rejectionPatterns") || {};
            
            return aSuggestions.filter(function (oSuggestion) {
                var sPatternKey = oSuggestion.type + "_" + oSuggestion.action;
                var aRejectedPatterns = oRejectionPatterns[sPatternKey] || [];
                
                // Check if this specific suggestion was rejected
                var bRejected = aRejectedPatterns.some(function (oRejected) {
        ...
        _filterSuppressedSuggestions: function (aSuggestions, sFieldName) {
            var oSuppressionData = this.getModel("view").getProperty("/suggestions/temporarySuppression") || {};
            var iCurrentTime = Date.now();
            
            return aSuggestions.filter(function (oSuggestion) {
                var sKey = sFieldName + "_" + oSuggestion.type;
                var oSuppression = oSuppressionData[sKey];
                
                if (!oSuppression) return true;
        ...
        _generateCompletionSuggestions: function (sFieldName, sCurrentValue) {
            var aSuggestions = [];
            var oPatterns = this._suggestionPatterns[sFieldName];
            ...
            if (oPatterns.completionSuggestions) {
                oPatterns.completionSuggestions.forEach(function (oSuggestionConfig) {
                    var sSuggestionValue = typeof oSuggestionConfig === 'string' ? 
            ...
            if (oPatterns.advancedSuggestions && sCurrentValue.length > 0) {
                oPatterns.advancedSuggestions.forEach(function (oAdvanced) {
                    var bTriggerMatch = !oAdvanced.trigger || 
        ...
        _generateValidationSuggestions: function (sFieldName, sCurrentValue, oProjectData) {
            ...
            var oValidationResult = this._validateField(sFieldName, sCurrentValue, oProjectData);
            
            if (!oValidationResult.isValid && oPatterns && oPatterns.validationTips) {
                oPatterns.validationTips.forEach(function (sTip, iIndex) {
                    aSuggestions.push({
                        type: "validation",
        ...
        _generateOptimizationSuggestions: function (sFieldName, sCurrentValue, oProjectData) {
            ...
            if (oPatterns && oPatterns.optimizationSuggestions) {
                oPatterns.optimizationSuggestions.forEach(function (sSuggestion) {
                    aSuggestions.push({
                        type: "optimization",
            ...
            if (sFieldName === "budget" && oProjectData.startDate && oProjectData.endDate) {
                var iMonths = this._calculateProjectMonths(oProjectData.startDate, oProjectData.endDate);
                if (iMonths > 12) {
                    aSuggestions.push({
                        type: "optimization",
        ...
        _prioritizeSuggestions: function (aSuggestions) {
            ...
            return aSuggestions.sort(function (a, b) {
        ...
        _showSuggestionsPopover: function (sFieldName) {
            ...
            var oControl = this.byId(sFieldName === "name" ? "projectNameInput" : 
        ...
        onAcceptSuggestion: function (oEvent) {
            var oSuggestion = oEvent.getSource().getBindingContext("view").getObject();
            var sFieldName = this._getCurrentSuggestionField();
        ...
        _getCurrentSuggestionField: function () {
            ...
            var sSuggestionField = this.getModel("view").getProperty("/suggestions/activeSuggestion");
            if (!sSuggestionField) {
                // Fallback: detect from popover position or last focused field
                var oFocusedControl = document.activeElement;
                var sControlId = oFocusedControl ? oFocusedControl.id : "";
                
                if (sControlId.includes("projectNameInput")) return "name";
                if (sControlId.includes("projectDescInput")) return "description";
                if (sControlId.includes("costCenterInput")) return "costCenter";
                if (sControlId.includes("budgetInput")) return "budget";
                if (sControlId.includes("startDatePicker")) return "startDate";
                if (sControlId.includes("endDatePicker")) return "endDate";
            }
            return sSuggestionField;
        },
        
        _processSuggestionAcceptance: function (oSuggestion, sFieldName) {
            var oAcceptanceResult = {
                success: false,
                originalValue: null,
                newValue: null,
            ...
            try {
                ...
                var oControl = this._getFieldControl(sFieldName);
                if (!oControl) {
        ...
        _getFieldControl: function (sFieldName) {
            ...
            var sControlId = mFieldControlMap[sFieldName];
        ...
        _validateSuggestionApplicability: function (oSuggestion, sFieldName, oControl) {
            ...
            var sFieldType = this._getFieldType(oControl);
        ...
        _getFieldType: function (oControl) {
            ...
            if (sControlName.includes("Input")) return "text";
            if (sControlName.includes("TextArea")) return "textarea";
            if (sControlName.includes("DatePicker")) return "date";
            if (sControlName.includes("ComboBox")) return "select";
            return "unknown";
        },
        
        _isSuggestionCompatible: function (oSuggestion, sFieldType) {
            var mCompatibilityMatrix = {
            ...
            var aCompatibleActions = mCompatibilityMatrix[sFieldType] || ["info"];
        ...
        _applySuggestionReplace: function (oSuggestion, oControl, oResult) {
            var sNewValue = oSuggestion.value || "";
        ...
        _applySuggestionAppend: function (oSuggestion, oControl, oResult) {
            var sCurrentValue = oResult.originalValue || "";
            var sAppendValue = oSuggestion.value || "";
            var sNewValue = sCurrentValue + sAppendValue;
        ...
        _applySuggestionInsert: function (oSuggestion, oControl, oResult) {
            var sCurrentValue = oResult.originalValue || "";
            var iInsertPosition = oSuggestion.insertPosition || sCurrentValue.length;
            var sInsertValue = oSuggestion.value || "";
            
            var sNewValue = sCurrentValue.substring(0, iInsertPosition) + 
                           sInsertValue + 
        ...
        _applySuggestionModify: function (oSuggestion, oControl, oResult) {
            var sCurrentValue = oResult.originalValue || "";
            var sPattern = oSuggestion.modifyPattern || "";
            var sReplacement = oSuggestion.value || "";
            
            var sNewValue = sCurrentValue.replace(new RegExp(sPattern, 'g'), sReplacement);
        ...
        _validateAppliedSuggestion: function (oControl, oResult) {
            ...
            var sFieldName = oResult.fieldName;
            var sNewValue = oResult.newValue;
            var oProjectData = this.getModel("project").getData();
            
            var oValidationResult = this._validateField(sFieldName, sNewValue, oProjectData);
            if (!oValidationResult.isValid) {
        ...
        _recordSuggestionAcceptance: function (oResult) {
            ...
            var oFeedback = this.getModel("view").getProperty("/suggestions/feedback");
            
            oFeedback.accepted = oFeedback.accepted || [];
            oFeedback.applied = oFeedback.applied || [];
            
            var oAcceptanceRecord = {
        ...
        _updateUIAfterSuggestionAcceptance: function (oResult) {
            ...
            var sMessage = "Suggestion applied: " + oResult.suggestion.title;
        ...
        _offerSuggestionUndo: function (oResult) {
            ...
            var oUndoBar = new sap.m.MessageStrip({
        ...
        _getAlternativeSuggestions: function (oRejectedSuggestion) {
            var aSuggestions = this.getModel("view").getProperty("/suggestions/currentSuggestions");
            return aSuggestions.filter(function (oSugg) {
        ...
        _handleActionableSuggestionRejection: function (oContext) {
            var oSuggestion = oContext.suggestion;
            
            // Show rejection reason dialog for actionable suggestions
            this._showRejectionReasonDialog(oContext, function (sReason, bBlockSimilar) {
            ...
            var oDialog = new sap.m.Dialog({
                ...
                buttons: [
                    new sap.m.Button({
                        ...
                        type: "Emphasized",
                        press: function () {
                            var sSelectedReason = sap.ui.getCore().byId("rejectionReasonGroup").getSelectedButton()?.getKey() || "no_reason";
                            var bBlockSimilar = sap.ui.getCore().byId("blockSimilarCheckbox").getSelected();
        ...
        _removeSuggestionFromList: function (oSuggestion) {
            var aSuggestions = this.getModel("view").getProperty("/suggestions/currentSuggestions");
            var iIndex = aSuggestions.indexOf(oSuggestion);
            if (iIndex > -1) {
        ...
        _addToRejectionPatterns: function (oSuggestion, sReason) {
            var oPatterns = this.getModel("view").getProperty("/suggestions/rejectionPatterns") || {};
        ...
        _updateUserPreferencesFromRejection: function (oRecord) {
            var oPrefs = this.getModel("view").getProperty("/suggestions/userPreferences");
            ...
            switch (oRecord.rejectionReason) {
                ...
                case "not_relevant":
                    ...
                    var sType = oRecord.suggestion.type;
                    if (sType === "completion") {
        ...
        _incorporateImprovedSuggestions: function (aImprovedSuggestions) {
            ...
            var aCurrent = this.getModel("view").getProperty("/suggestions/currentSuggestions");
            
            aImprovedSuggestions.forEach(function (oImproved) {
        ...
        _getCompletedFieldsCount: function () {
            var oProject = this.getModel("project").getData();
            var aFieldNames = ["name", "description", "startDate", "endDate", "budget", "costCenter"];
            
            return aFieldNames.filter(function (sField) {
        ...
        onViewLearningAnalytics: function () {
            var oLearningData = this._generateLearningAnalytics();
        ...
        _generateLearningAnalytics: function () {
            var oFeedback = this.getModel("view").getProperty("/suggestions/feedback") || { accepted: [], rejected: [], applied: [] };
            var oPrefs = this.getModel("view").getProperty("/suggestions/userPreferences") || {};
            
            // Calculate learning metrics
            var iTotalInteractions = (oFeedback.accepted || []).length + (oFeedback.rejected || []).length;
            var fAcceptanceRate = iTotalInteractions > 0 ? ((oFeedback.accepted || []).length / iTotalInteractions) * 100 : 0;
            
            // Analyze rejection patterns
            var oRejectionAnalysis = this._analyzeRejectionPatterns(oFeedback.rejected || []);
            
            // Analyze acceptance patterns  
            var oAcceptanceAnalysis = this._analyzeAcceptancePatterns(oFeedback.accepted || []);
            
            // Generate improvement recommendations
            var aImprovements = this._generateImprovementRecommendations(oRejectionAnalysis, oAcceptanceAnalysis);
            
            // User engagement metrics
            var oEngagement = this._calculateEngagementMetrics(oFeedback);
            
            // Learning evolution over time
            var oEvolution = this._analyzeLearningEvolution(oFeedback);
        ...
        _analyzeRejectionPatterns: function (aRejected) {
            var oReasons = {};
            var oFieldTypes = {};
            var oSuggestionTypes = {};
            var oTimePatterns = {};
            
            aRejected.forEach(function (oRejection) {
                // Reason analysis
                ...
                var sType = oRejection.suggestion ? oRejection.suggestion.type : 'unknown';
                oSuggestionTypes[sType] = (oSuggestionTypes[sType] || 0) + 1;
                
                // Time pattern analysis
                if (oRejection.timestamp) {
                    var iHour = new Date(oRejection.timestamp).getHours();
                    var sTimeSlot = iHour < 12 ? 'morning' : iHour < 18 ? 'afternoon' : 'evening';
        ...
        _analyzeAcceptancePatterns: function (aAccepted) {
            ...
            var oSuggestionTypes = {};
            var oTimePatterns = {};
            var oConfidenceRanges = { low: 0, medium: 0, high: 0 };
            
            aAccepted.forEach(function (oAcceptance) {
                // Field type analysis
                oFieldTypes[oAcceptance.fieldName] = (oFieldTypes[oAcceptance.fieldName] || 0) + 1;
                
                // Suggestion type analysis
                var sType = oAcceptance.suggestion ? oAcceptance.suggestion.type : 'unknown';
                oSuggestionTypes[sType] = (oSuggestionTypes[sType] || 0) + 1;
                
                // Time pattern analysis
                if (oAcceptance.timestamp) {
                    var iHour = new Date(oAcceptance.timestamp).getHours();
                    var sTimeSlot = iHour < 12 ? 'morning' : iHour < 18 ? 'afternoon' : 'evening';
                    oTimePatterns[sTimeSlot] = (oTimePatterns[sTimeSlot] || 0) + 1;
                }
                ...
                if (oAcceptance.suggestion && oAcceptance.suggestion.confidence) {
                    var fConf = oAcceptance.suggestion.confidence;
                    if (fConf < 0.6) {
        ...
        _calculateEngagementMetrics: function (oFeedback) {
            var iTotalInteractions = (oFeedback.accepted || []).length + (oFeedback.rejected || []).length;
            var iAppliedSuggestions = (oFeedback.applied || []).length;
            
            // Calculate session-based metrics
            var oSessions = this._analyzeUserSessions(oFeedback);
        ...
        _analyzeUserSessions: function (oFeedback) {
            var oSessions = {};
            var allInteractions = [].concat(oFeedback.accepted || [], oFeedback.rejected || []);
            
            allInteractions.forEach(function (oInteraction) {
        ...
        _analyzeLearningEvolution: function (oFeedback) {
            var allInteractions = [].concat(
                (oFeedback.accepted || []).map(function(item) { return Object.assign({}, item, {type: 'accepted'}); }),
                (oFeedback.rejected || []).map(function(item) { return Object.assign({}, item, {type: 'rejected'}); })
            );
            ...
            var aEvolution = [];
            var iRunningAccepted = 0;
            var iRunningRejected = 0;
            
            allInteractions.forEach(function (oInteraction, iIndex) {
                ...
                if ((iIndex + 1) % 5 === 0) { // Every 5 interactions
                    var iTotalSoFar = iRunningAccepted + iRunningRejected;
                    aEvolution.push({
        ...
        _calculateSystemConfidence: function (oFeedback) {
            var iTotalInteractions = (oFeedback.accepted || []).length + (oFeedback.rejected || []).length;
            var fAcceptanceRate = iTotalInteractions > 0 ? ((oFeedback.accepted || []).length / iTotalInteractions) : 0;
            
            // Confidence increases with more interactions and higher acceptance rate
            var fInteractionWeight = Math.min(iTotalInteractions / 50, 1); // Max weight at 50 interactions
            var fAcceptanceWeight = fAcceptanceRate;
            
            return Math.round(((fInteractionWeight * 0.3) + (fAcceptanceWeight * 0.7)) * 100);
        },
        
        _calculateLearningScore: function (oFeedback, oEngagement) {
            var fAcceptanceRate = ((oFeedback.accepted || []).length) / 
                Math.max(((oFeedback.accepted || []).length + (oFeedback.rejected || []).length), 1);
            var fEngagementScore = Math.min(oEngagement.totalInteractions / 20, 1); // Max at 20 interactions
            var fApplicationScore = Math.min((oFeedback.applied || []).length / 10, 1); // Max at 10 applications
        ...
        _calculateEngagementLevel: function (iTotalInteractions, iAppliedSuggestions) {
            if (iTotalInteractions === 0) return 'none';
            if (iTotalInteractions < 5) return 'low';
            if (iTotalInteractions < 15) return 'moderate';
            if (iTotalInteractions < 30) return 'high';
            return 'expert';
        },
        ...
        _determineUserType: function (oFeedback) {
            ...
            if (fAcceptanceRate > 0.8) return 'accepter';
            if (fAcceptanceRate < 0.3) return 'skeptical';
            return 'balanced';
        },
        ...
        _calculateAvgSessionLength: function (oSessions) {
            ...
            for (var sSessionId in oSessions) {
                var oSession = oSessions[sSessionId];
        ...
        _calculateOverallTrend: function (aEvolution) {
            if (aEvolution.length < 2) return 'insufficient_data';
            
            var fFirstRate = aEvolution[0].acceptanceRate;
            var fLastRate = aEvolution[aEvolution.length - 1].acceptanceRate;
            var fDifference = fLastRate - fFirstRate;
            
            if (fDifference > 10) return 'improving';
            if (fDifference < -10) return 'declining';
            return 'stable';
        },
        
        _identifyImprovementPeriods: function (aEvolution) {
            var aPeriods = [];
            var iCurrentStart = null;
            
            for (var i = 1; i < aEvolution.length; i++) {
        ...
        onExportLearningData: function () {
            var oLearningData = this.getModel("view").getProperty("/learningAnalytics");
            
            // Create comprehensive learning report
            var sReportContent = this._generateLearningReport(oLearningData);
            
            // Download as CSV
            var oBlob = new Blob([sReportContent], { type: 'text/csv;charset=utf-8;' });
            var sFileName = 'ai_learning_analytics_' + new Date().toISOString().split('T')[0] + '.csv';
        ...
        _generateLearningReport: function (oData) {
            ...
            for (var sReason in oData.rejectionAnalysis.reasonBreakdown) {
            ...
            oData.improvements.forEach(function (oImprovement) {
        ...
        _startInsightsAutoRefresh: function () {
            var iInterval = this.getModel("view").getProperty("/insights/refreshInterval");
            
            this._insightsEngine.analysisTimer = setInterval(function () {
        ...
        _generateInsights: function () {
            var oProjectData = this.getModel("project").getData();
            if (!oProjectData || !oProjectData.projectId) return;
            ...
            var fConfidenceThreshold = this.getModel("view").getProperty("/insights/analysisConfig/confidenceThreshold");
            aInsights = aInsights.filter(function (oInsight) {
        ...
        _generateRiskInsights: function (oProjectData) {
            ...
            if (oProjectData.budget && oProjectData.estimatedCost) {
                var fBudgetUtilization = oProjectData.estimatedCost / oProjectData.budget;
            ...
            if (oProjectData.startDate && oProjectData.endDate) {
                var dStart = new Date(oProjectData.startDate);
                var dEnd = new Date(oProjectData.endDate);
                var iDuration = Math.ceil((dEnd - dStart) / (1000 * 60 * 60 * 24));
        ...
        _generateOptimizationInsights: function (oProjectData) {
            ...
            if (oProjectData.agents && oProjectData.agents.length > 0) {
                var iActiveAgents = oProjectData.agents.filter(function (agent) {
        ...
        _generatePredictionInsights: function (oProjectData) {
            ...
            if (oProjectData.startDate && oProjectData.endDate) {
                ...
                if (dStart <= dToday && dEnd >= dToday) {
                    ...
                    var fProgress = (dToday - dStart) / (dEnd - dStart);
                    var fActualProgress = oProjectData.progress / 100;
                    var fVariance = fActualProgress - fProgress;
        ...
        _prioritizeInsights: function (aInsights) {
            ...
            return aInsights.sort(function (a, b) {
                var mSeverityWeight = {
                    "critical": 4,
                    "warning": 3,
                    "info": 2
                ...
                var iSeverityA = mSeverityWeight[a.severity] || 1;
                var iSeverityB = mSeverityWeight[b.severity] || 1;
                ...
                var iCategoryA = this.getModel("view").getProperty("/insights/insightCategories/" + a.category + "/priority") || 10;
                var iCategoryB = this.getModel("view").getProperty("/insights/insightCategories/" + b.category + "/priority") || 10;
        ...
        onExportInsights: function () {
            var aInsights = this.getModel("view").getProperty("/insights/currentInsights");
        ...
        onSaveInsightsConfig: function () {
            ...
            var oConfig = this.getModel("view").getProperty("/insights");
        ...
        _exportInsightsToCSV: function (aInsights) {
            var aCSVData = [];
            var aHeaders = ["Category", "Type", "Title", "Severity", "Confidence", "Impact", "Description", "Recommendations"];
            aCSVData.push(aHeaders.join(","));
            
            aInsights.forEach(function (oInsight) {
                var aRow = [
                    oInsight.category,
                    oInsight.type,
        ...
        _recordSuggestionFeedback: function (oSuggestion, sAction) {
            var oFeedback = this.getModel("view").getProperty("/suggestions/feedback");
            var aFeedbackArray = oFeedback[sAction] || [];
        ...
        onToggleSuggestions: function () {
            var bEnabled = this.getModel("view").getProperty("/suggestions/enabled");
        ...
        onSuggestionsSettingChange: function () {
            ...
            var bEnabled = this.getModel("view").getProperty("/suggestions/userPreferences/enableAI");
            this.getModel("view").setProperty("/suggestions/enabled", bEnabled);
        },
        ...
        onSaveSuggestionsConfig: function () {
            ...
            var oConfig = this.getModel("view").getProperty("/suggestions");
        ...
        _showSuggestionPreview: function (oSuggestion, sFieldName) {
            ...
            var sCurrentValue = oControl.getValue();
            var sPreviewValue = this._calculatePreviewValue(oSuggestion, sCurrentValue);
            
            // Show preview dialog
            var sDialogContent = "Current value: \"" + (sCurrentValue || "(empty)") + "\"\n\n" +
        ...
        _calculatePreviewValue: function (oSuggestion, sCurrentValue) {
            switch (oSuggestion.action) {
                ...
                case "insert":
                    var iPos = oSuggestion.insertPosition || (sCurrentValue ? sCurrentValue.length : 0);
