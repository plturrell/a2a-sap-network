...
Tests HANA (primary) + SQLite (fallback) with AI enhancement
"""
...
class ORDDualDatabaseTester:
    ...
    async def _test_service_initialization(self):
        ...
        try:
            ...
            if self.service.storage:
                self._log_result("Dual-Database Storage", True, "✅ HANA + SQLite storage available")
            else:
    ...
    async def _test_dual_database_storage(self, registration_id: str):
        """Test 3: Verify data is stored in both HANA and SQLite"""
        print("\n🔬 Test 3: Dual-Database Storage Verification")
        print("-" * 50)
        ...
        try:
            ...
            # Test SQLite storage directly  
            sqlite_result = await self.service.storage._get_registration_sqlite(registration_id)
            if sqlite_result:
                self._log_result("SQLite Storage", True, "✅ Data replicated to SQLite (fallback)")
            else:
                self._log_result("SQLite Storage", False, "⚠️ Data not found in SQLite")
    ...
    def _print_test_summary(self):
        ...
        if success_rate >= 90:
            ...
            print("✅ HANA primary + SQLite fallback with AI enhancement is operational")
