sap.ui.define([
    ...
    "sap/m/MessageToast"
], (BaseObject, CrossApplicationNavigation, MessageToast) => {
    "use strict";
/* No global declarations needed - sap and URL are built-in globals */
    ...
    return BaseObject.extend("com.sap.a2a.developerportal.services.NavigationService", {
        ...
        navigateToSemanticObject: function (sSemanticObject, sAction, mParameters, mInnerAppData) {
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    const oSemanticObject = that._mSemanticObjects[sSemanticObject];
                    if (!oSemanticObject) {
                        throw new Error(`Semantic object '${  sSemanticObject  }' not found`);
                    }
                    
                    const oAction = oSemanticObject[sAction];
                    if (!oAction) {
                    ...
                    if (that._oCrossAppNav) {
                        const sHash = that._oCrossAppNav.hrefForExternal({
                            target: {
        ...
        _navigateInternally: function (oAction, mParameters) {
            ...
            if (!oRouter) {
                ...
                const aViews = sap.ui.getCore().byFieldGroupId("view");
                if (aViews.length > 0) {
        ...
        getSupportedActions: function (sSemanticObject) {
            const oSemanticObject = this._mSemanticObjects[sSemanticObject];
        ...
        getNavigationIntent: function (sSemanticObject, sAction, mParameters) {
            if (this._oCrossAppNav) {
                ...
                const oAction = this._mSemanticObjects[sSemanticObject] && 
                             this._mSemanticObjects[sSemanticObject][sAction];
                
                if (oAction) {
                    let sHash = `#${  oAction.route}`;
        ...
        createExternalUrl: function (sSemanticObject, sAction, mParameters) {
            ...
            const sIntent = this.getNavigationIntent(sSemanticObject, sAction, mParameters);
