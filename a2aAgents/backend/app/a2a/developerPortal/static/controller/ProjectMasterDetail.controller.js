...
/* global localStorage */
    ...
    return Controller.extend("com.sap.a2a.developerportal.controller.ProjectMasterDetail", {
            ...
            this.getView().addEventDelegate({
                onAfterRendering: function () {
                    ...
                    if (oFCL && !this._resizeHandleAdded) {
                        const $resizeHandle = $('<div class="a2a-fcl-resize-handle"></div>');
        ...
        _loadProjectDetails: function (sProjectId) {
            ...
            const iUpdateCount = this.oDetailModel.getProperty("/updateCount") || 0;
        ...
        onSearch: function (oEvent) {
            ...
            setTimeout(() => {
                ...
                const iResultCount = oList.getItems().length;
        ...
        _deployProject: function (sProjectId) {
            ...
            let iProgress = 0;
            const oProgressInterval = setInterval(() => {
                iProgress += 10;
...
return "";
        ...
        getUpdateStatistics: function () {
            ...
            const iTotal = this._detailUpdateHistory.length;
            const iSuccessful = this._detailUpdateHistory.filter((update) => {
                return update.success;
            }).length;
            const iDurationSum = this._detailUpdateHistory.reduce((sum, update) => {
        ...
        forceDetailRefresh: function () {
            const sCurrentProjectId = this.oDetailModel.getProperty("/project_id");
        ...
        _initializeKeyboardNavigation: function () {
            ...
            this.getView().addEventDelegate({
                onAfterRendering: function () {
                    ...
                    const oMasterList = this.byId("masterList");
                    if (oMasterList) {
...
return;
            ...
            const oFocusedItem = oList.getItems().find((oItem) => {
                return oItem.$().hasClass("sapMLIBFocused");
            });
            
            if (oFocusedItem) {
                const sProjectId = oFocusedItem.getCustomData()[0].getValue();
        ...
        _navigateToDetail: function () {
            ...
            if (sCurrentLayout !== LayoutType.OneColumn) {
                ...
                const oDetailPage = this.byId("detailPage");
                if (oDetailPage) {
                    const $firstButton = oDetailPage.$().find("button:visible:first");
...
return;
            ...
            const iCurrentIndex = oSelectedItem ? aItems.indexOf(oSelectedItem) : -1;
            ...
            if (iNewIndex !== iCurrentIndex && aItems[iNewIndex]) {
                const sProjectId = aItems[iNewIndex].getCustomData()[0].getValue();
                this.oRouter.navTo("projectMasterDetail", {
                    projectId: sProjectId
                });
            }
        ...
        _preloadProjectDetails: function (sProjectId) {
            ...
            const sCacheKey = `project_${  sProjectId}`;
        ...
        _updateErrorModel: function () {
            const iErrorCount = this._errorHistory.length;
            const oLastError = iErrorCount > 0 ? this._errorHistory[iErrorCount - 1] : null;
        ...
        _categorizeError: function (oError) {
            const oContext = oError.context;
            const iStatus = oContext.status;
        ...
        _showErrorToUser: function (oError) {
            const sUserMessage = this._getUserFriendlyMessage(oError);
            const sTitle = this._getErrorTitle(oError);
        ...
        _getErrorDetails: function (oError) {
            let sDetails = `Error ID: ${  oError.id  }\n`;
        ...
        _retryLastOperation: function (oError) {
            ...
            if (oContext.url && oContext.url.includes("/projects")) {
                if (oContext.url.includes("/deploy")) {
                    // Retry deployment
                    const sProjectId = this._extractProjectIdFromUrl(oContext.url);
        ...
        _extractProjectIdFromUrl: function (sUrl) {
            const aMatches = sUrl.match(/\/projects\/([^/]+)/);
        ...
        getErrorStatistics: function () {
            ...
            this._errorHistory.forEach((oError) => {
                switch (oError.category) {
                    case "network":
        ...
        onRetryError: function (oEvent) {
            const oBindingContext = oEvent.getSource().getBindingContext("errorDialog");
            const oError = oBindingContext.getObject();
            this._retryLastOperation(oError);
        },
...
return "";
            ...
            const oDate = new Date(oTimestamp);
...
return "";
            ...
            let sDetails = `<strong>Error Message:</strong><br/>${  oError.message  }<br/><br/>`;
...
return;
            ...
            const iCurrentX = oEvent.pageX || oEvent.touches[0].pageX;
            const iDelta = iCurrentX - this._iStartX;
            let iNewWidth = this._iStartWidth + iDelta;
            
            // Apply constraints
            const iMinWidth = this.oLayoutModel.getProperty("/minColumnWidth");
            const iMaxWidth = this.oLayoutModel.getProperty("/maxBeginColumnWidth");
            
            iNewWidth = Math.max(iMinWidth, Math.min(iMaxWidth, iNewWidth));
            
            // Calculate percentage
            const iTotalWidth = this.byId("fcl").$().width();
            const iBeginPercent = (iNewWidth / iTotalWidth) * 100;
            const iMidPercent = 100 - iBeginPercent;
        ...
        _applyColumnWidths: function () {
            const oFCL = this.byId("fcl");
            const oWidths = this.oLayoutModel.getProperty("/columnWidths");
        ...
        _loadColumnWidthPreference: function () {
            const sWidths = localStorage.getItem("a2a.fcl.columnWidths");
            if (sWidths) {
                try {
                    const oWidths = JSON.parse(sWidths);
        ...
        _checkResponsiveBreakpoint: function () {
            const iWindowWidth = $(window).width();
            const oDeviceModel = this.getView().getModel("device");
            let oCurrentBreakpoint = null;
            let sBreakpointName = "";
...
return;
        ...
        _ensureItemVisible: function (oItem) {
            ...
            setTimeout(() => {
                const oItemDom = oItem.getDomRef();
                if (oItemDom) {
        ...
        // Duplicate _onResize method commented out
        // _onResize: function (oEvent) {
        //     if (!this._bResizing) {
        //         return;
        //     }
