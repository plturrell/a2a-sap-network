sap.ui.define([
  ...
  'use strict';
  /* global WebSocket, Blob, URL, FormData */
  ...
  return Controller.extend('a2a.portal.controller.ProjectDetail', {
    ...
    _loadProjectFiles: function (sProjectId) {
      jQuery.ajax({
        ...
        method: 'GET',
        success: function (data) {
          const oFileModel = this.getView().getModel('files');
    ...
    _buildFileTree: function (aFiles) {
      ...
      aFiles.forEach((oFile) => {
        const sParentPath = oFile.path.substring(0, oFile.path.lastIndexOf('/'));
    ...
    onDeployProject: function () {
      const _sProjectId = this._sCurrentProjectId;
      const _oView = this.getView();
      ...
      const oDeployConfig = new JSONModel({
    ...
    onConfirmDeployProject: function () {
      const oDeployConfig = this._oDeployConfigDialog.getModel('deployConfig').getData();
    ...
    _updateDeploymentProgress: function (sDeploymentId) {
      const oProgressIndicator = sap.ui.getCore().byId('deployProgressIndicator');
      const oStatusText = sap.ui.getCore().byId('deployStatusText');
      const oStepsBox = sap.ui.getCore().byId('deploymentStepsBox');
      const oCloseButton = sap.ui.getCore().byId('deployCloseButton');
            
      const fnCheckProgress = function () {
        jQuery.ajax({
          ...
          success: function (data) {
            ...
            data.steps.forEach((step) => {
              const oStepItem = new sap.m.HBox({
                items: [
    ...
    onToggleFolder: function (oEvent) {
      const oTreeItem = oEvent.getParameter('item');
      const oContext = oTreeItem.getBindingContext();
    ...
    onFileSelectionChange: function (oEvent) {
      ...
      const aSelectedFiles = aSelectedItems.map((oItem) => {
    ...
    onConfirmCreateFile: function () {
      const oCreateModel = this._oCreateFileDialog.getModel('createFile');
      const sFileName = oCreateModel.getProperty('/fileName');
      const sFileType = oCreateModel.getProperty('/fileType');
      const sParentPath = oCreateModel.getProperty('/parentPath');
      const sFileContent = oCreateModel.getProperty('/fileContent');
      ...
      const sFullPath = sParentPath + (sParentPath.endsWith('/') ? '' : '/') + sFileName;
    ...
    onConfirmCreateFolder: function () {
      const oCreateModel = this._oCreateFolderDialog.getModel('createFolder');
      const sFolderName = oCreateModel.getProperty('/folderName');
      const sParentPath = oCreateModel.getProperty('/parentPath');
      ...
      const sFullPath = sParentPath + (sParentPath.endsWith('/') ? '' : '/') + sFolderName;
    ...
    onUploadFiles: function () {
      ...
      const oFileUpload = document.createElement('input');
    ...
    onOpenSelectedFile: function () {
      const oFileModel = this.getView().getModel('files');
      const aSelectedFiles = oFileModel.getProperty('/selectedFiles');
    ...
    _showFileContentDialog: function (oFileData) {
      ...
      jQuery.ajax({
        ...
        success: function (sContent) {
          ...
          const oContentModel = new JSONModel({
    ...
    _isEditableFile: function (sExtension) {
      const aEditableExtensions = ['txt', 'js', 'json', 'xml', 'html', 'css', 'md', 'yaml', 'yml'];
    ...
    _renameFile: function (sOldPath, sNewName) {
      const sParentPath = sOldPath.substring(0, sOldPath.lastIndexOf('/'));
      const sNewPath = sParentPath + (sParentPath ? '/' : '') + sNewName;
    ...
    onDeleteFiles: function () {
      ...
      const sMessage = `Are you sure you want to delete ${  aSelectedFiles.length  } selected file(s)?\n\nThis action cannot be undone.`;
    ...
    _deleteSelectedFiles: function (aFiles) {
      const aFilePaths = aFiles.map((oFile) => {
        return oFile.path;
      });
    ...
    _updateFolderContents: function (sFolderPath, aFiles) {
      const oFileModel = this.getView().getModel('files');
      const aFileTree = oFileModel.getProperty('/files');
            
      // Find and update the folder in the tree
      const oFolder = this._findFileInTree(aFileTree, sFolderPath);
      if (oFolder) {
    ...
    _findFileInTree: function (aFiles, sPath) {
      for (let i = 0; i < aFiles.length; i++) {
        ...
        if (aFiles[i].children && aFiles[i].children.length > 0) {
          const oFound = this._findFileInTree(aFiles[i].children, sPath);
          if (oFound) {
            return oFound;
          }
        }
    ...
    // Formatters
    // Duplicate formatDate method removed
    ...
    onSaveProject: function () {
      ...
      const oProjectData = oView.getModel().getData();
      const sProjectId = this._sCurrentProjectId;
    ...
    _updateBuildProgress: function (sBuildId) {
      const oProgressIndicator = sap.ui.getCore().byId('buildProgressIndicator');
      const oStatusText = sap.ui.getCore().byId('buildStatusText');
    ...
    onRunProject: function () {
      const _sProjectId = this._sCurrentProjectId;
      const _oProjectData = this.getView().getModel().getData();
      ...
      const oRunConfig = new JSONModel({
    ...
    onConfirmRunProject: function () {
      const oRunConfig = this._oRunConfigDialog.getModel('runConfig').getData();
      const sProjectId = this._sCurrentProjectId;
    ...
    _initializeTerminal: function () {
      const oTerminalModel = new JSONModel({
        sessions: [],
    ...
    _createTerminalSession: function () {
      const oTerminalModel = this.getView().getModel('terminal');
      const aSessions = oTerminalModel.getProperty('/sessions');
      const sSessionId = `terminal_${  Date.now()}`;
    ...
    _connectTerminalWebSocket: function (sSessionId) {
      ...
      oWebSocket.onerror = function (_error) {
    ...
    _updateSessionWebSocket: function (sSessionId, oWebSocket) {
      const oTerminalModel = this.getView().getModel('terminal');
      const aSessions = oTerminalModel.getProperty('/sessions');
      const oSession = aSessions.find((s) => {
        return s.id === sSessionId; 
      });
      if (oSession) {
    ...
    _appendTerminalOutput: function (sSessionId, sContent, sType) {
      ...
      if (oSession) {
        ...
        // Limit scrollback
        const iMaxLines = oTerminalModel.getProperty('/settings/scrollback');
        ...
        setTimeout(() => {
          const oContainer = this.byId('terminalContainer');
          if (oContainer) {
    ...
    onTerminalCommandSubmit: function (_oEvent) {
      const oTerminalModel = this.getView().getModel('terminal');
      const sCommand = oTerminalModel.getProperty('/currentCommand');
      const sActiveSessionId = oTerminalModel.getProperty('/activeSessionId');
      ...
      const aHistory = oTerminalModel.getProperty('/commandHistory');
    ...
    onExportTerminalOutput: function () {
      ...
      if (oSession) {
        const sOutput = oSession.output.map((o) => {
          return `[${  new Date(o.timestamp).toLocaleTimeString()  }] ${  o.content}`;
        }).join('\n');
                
        const blob = new Blob([sOutput], { type: 'text/plain' });
    ...
    _applyTerminalSettings: function () {
      const oTerminalModel = this.getView().getModel('terminal');
      const oSettings = oTerminalModel.getProperty('/settings');
            
      // Apply settings via CSS custom properties or direct styling
      const oTerminalContent = this.byId('terminalContent');
      if (oTerminalContent) {
