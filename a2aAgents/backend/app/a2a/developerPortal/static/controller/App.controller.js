sap.ui.define([
  ...
  '../mixin/OfflineMixin'
], (BaseController, MessageToast, MessageBox, Fragment, JSONModel, ResourceModel, PersonalizationMixin, OfflineMixin) => {
  'use strict';
  ...
  return BaseController.extend('a2a.portal.controller.App', {
    ...
    onInit: function () {
      ...
      const oHelpResourceModel = new ResourceModel({
    ...
    onNotificationBellPress: function () {
    ...
    onNotificationPress: function (oEvent) {
      const oBindingContext = oEvent.getSource().getBindingContext('notifications');
      ...
      if (oNotification.actions && oNotification.actions.length > 0) {
        const oPrimaryAction = oNotification.actions.find((action) => {
    ...
    onNotificationActionPress: function (oEvent) {
      ...
      const sActionType = oButton.data('actionType');
      const sTarget = oButton.data('target');
      const sActionId = oButton.data('actionId');
    ...
    formatRelativeTime: function (sTimestamp) {
      ...
      const iHours = Math.floor(iDiff / (1000 * 60 * 60));
    ...
    _initializeSettingsModel: function () {
      ...
      const oCurrentSettings = this._loadUserSettings();
            
      const oSettingsModel = new JSONModel(oCurrentSettings);
    ...
    _loadUserSettings: function () {
      ...
      const sStoredSettings = localStorage.getItem('a2a_user_settings');
      ...
      if (sStoredSettings) {
        try {
          const oParsedSettings = JSON.parse(sStoredSettings);
    ...
    _resetToDefaults: function () {
      let oDefaultSettings = this._loadUserSettings();
    ...
    _logSettingsChange: function (sAction, oSettings) {
      ...
      const aAuditLogs = JSON.parse(sessionStorage.getItem('audit_logs') || '[]');
    ...
    _showApplicationLogs: function () {
      const sLogs = `=== A2A Agents Application Logs ===\n\n` +
    ...
    onNavigate: function (_oEvent) {
      // Handle navigation events
    },

    onAfterNavigate: function (_oEvent) {
    ...
    _addHelpButtonToShellBar: function () {
      ...
      const aItems = oShellBar.getProfile() ? oShellBar.getProfile().getMenu().getItems() : [];
      const bHelpExists = aItems.some((oItem) => {
        return oItem.getText && oItem.getText() === 'Help & Support';
      });
            
      if (!bHelpExists && oShellBar.getProfile()) {
        // Add help menu item
        const oHelpMenuItem = new sap.m.MenuItem({
          text: 'Help & Support',
      ...
      const oHelpButton = new sap.m.Button({
    ...
    /**
         ...
         * @public
         */
    onShowHelpMenu: function (oEvent) {
      const that = this;
      ...
      this._oHelpMenu.openBy(oEvent.target);
    },
    ...
    onShowKeyboardShortcuts: function () {
      const oShortcutsDialog = new sap.m.Dialog({
