  ...
  return Controller.extend('a2a.portal.controller.AgentBuilder', {
    ...
    onCapabilitySelect: function (oEvent) {
      ...
      if (bSelected) {
        ...
        aSelected = aSelected.filter((item) => {
          return item.id !== oCapability.id; 
        });
      }
    ...
    onAddSelectedCapabilities: function () {
      ...
      aNewSkills.forEach((newSkill) => {
        const bExists = aCurrentSkills.some((skill) => {
    ...
    _addPredefinedCapabilities: function (aCapabilityIds) {
      ...
      aCapabilityIds.forEach((sId) => {
        ...
        if (oCapability && !aSelected.some((sel) => {
          return sel.id === sId; 
        })) {
          aSelected.push(oCapability);
        }
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          if (data.unique) {
            ...
            let sMessage = '';
            if (!data.nameUnique) {
              sMessage += 'Agent name already exists. ';
            }
            if (!data.idUnique) {
              sMessage += 'Agent ID already exists.';
            }
...
`; 
    ...
    onUpdateTags: function (oEvent) {
      ...
      const aTags = aTokens.map((oToken) => {
        return {
          key: oToken.getKey(),
          text: oToken.getText()
        };
      ...
      const sTagsString = aTags.map((oTag) => {
        return oTag.text;
      }).join(', ');
    ...
    onDeleteTag: function (oEvent) {
      ...
      const aFilteredTags = aTags.filter((oTag) => {
      ...
      // Update tags string
      const sTagsString = aFilteredTags.map((oTag) => {
        return oTag.text;
      }).join(', ');
    ...
    _autoFillMetadata: function () {
      ...
      if (!oAgent.metadata.tags.length) {
        ...
        if (aSuggestedTags.length) {
          ...
          const sTagsString = aSuggestedTags.map((oTag) => {
            return oTag.text;
          }).join(', ');
          oAgentModel.setProperty('/metadata/tagsString', sTagsString);
        }
    ...
    _startBuildProcess: function () {
      ...
      this._executeBuildStep(1, 'Validating configuration', () => {
        return this._validateBuildConfiguration();
      })
        .then(() => {
          return this._executeBuildStep(2, 'Generating source code', () => {
            return this._generateBuildCode();
          });
        })
        .then(() => {
          return this._executeBuildStep(3, 'Resolving dependencies', () => {
            return this._resolveDependencies();
          });
        })
        .then(() => {
          return this._executeBuildStep(4, 'Compiling agent', () => {
            return this._compileAgent();
          });
        })
        .then(() => {
          return this._executeBuildStep(5, 'Running tests', () => {
            return this._runBuildTests();
          });
        })
        .then(() => {
          return this._executeBuildStep(6, 'Generating artifacts', () => {
    ...
    _resolveDependencies: function () {
      ...
      if (oAgentData.skills) {
        oAgentData.skills.forEach((skill) => {
