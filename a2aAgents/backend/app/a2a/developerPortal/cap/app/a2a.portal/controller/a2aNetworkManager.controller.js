...
], (Controller, MessageBox, MessageToast, JSONModel, Fragment, BusyIndicator) => {
    "use strict";
/* global WebSocket, localStorage */
    ...
    return Controller.extend("a2a.portal.controller.A2ANetworkManager", {
        ...
        _initializeModels: function() {
            ...
            const oNetworkModel = new JSONModel({
                connected: false,
                network: "mainnet",
                chainId: null,
                blockNumber: null,
            ...
            // Agents model
            const oAgentsModel = new JSONModel({
                agents: [],
                totalAgents: 0,
                selectedAgent: null,
                loading: false
            ...
            // Messages model
            const oMessagesModel = new JSONModel({
                messages: [],
                unreadCount: 0
            });
            ...
            // Webhooks model
            const oWebhooksModel = new JSONModel({
                subscriptions: [],
                eventTypes: [
        ...
        _connectToA2ANetwork: function() {
            ...
            const sPrivateKey = localStorage.getItem("a2a_private_key");
            const sNetwork = localStorage.getItem("a2a_network") || "mainnet";
            ...
            jQuery.ajax({
                ...
                success: function(data) {
                    ...
                    const oNetworkModel = this.getView().getModel("network");
        ...
        _loadAgents: function() {
            const oAgentsModel = this.getView().getModel("agents");
        ...
        onRegisterAgentConfirm: function() {
            const oModel = this._oRegisterDialog.getModel("newAgent");
        ...
        onAgentSelect: function(oEvent) {
            ...
            const oAgent = oItem.getBindingContext("agents").getObject();
        ...
        _showAgentDetail: function(oAgent) {
            if (!this._oAgentDetailDialog) {
                Fragment.load({
                    ...
                    const oModel = new JSONModel(oAgent);
                    this._oAgentDetailDialog.setModel(oModel, "agentDetail");
                    
                    this._oAgentDetailDialog.open();
                });
            } else {
                const oModel = this._oAgentDetailDialog.getModel("agentDetail");
        ...
        onSendMessage: function() {
            const oAgent = this._oAgentDetailDialog.getModel("agentDetail").getData();
            ...
            if (!this._oMessageDialog) {
                ...
                const oModel = this._oMessageDialog.getModel("message");
        ...
        _loadWebhooks: function() {
            jQuery.ajax({
                ...
                type: "GET",
                success: function(data) {
                    const oWebhooksModel = this.getView().getModel("webhooks");
        ...
        onAddWebhookConfirm: function() {
            const oModel = this._oAddWebhookDialog.getModel("newWebhook");
            const oData = oModel.getData();
        ...
        onDeleteWebhook: function(oEvent) {
            const oItem = oEvent.getParameter("listItem");
            const oWebhook = oItem.getBindingContext("webhooks").getObject();
        ...
        onSaveNetworkSettings: function() {
            const oModel = this._oNetworkSettingsDialog.getModel("settings");
            const oData = oModel.getData();
