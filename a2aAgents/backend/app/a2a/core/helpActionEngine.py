...
class HelpResponseParser:
    ...
    def parse_advisor_response(self, help_response: Dict[str, Any]) -> ActionPlan:
        ...
        try:
            ...
            logger.info(
                f"📋 Parsed help response into action plan with {len(action_plan.actions)} actions"
            )
            return action_plan
    ...
    def _extract_actions_from_text(
        ...
        if any(
            word in answer_lower for word in ["alternative", "different approach", "workaround"]
        ):
            actions.append(
                HelpAction(
...
class HelpActionExecutor:
    ...
    async def execute_action_plan(
        ...
        try:
            logger.info(
                f"🚀 Executing action plan {action_plan.plan_id} with {len(action_plan.actions)} actions"
            )
            ...
            for action in sorted_actions:
                try:
                    action_result = await self._execute_single_action(
                        action, original_operation, operation_context or {}
                    )
                    ...
                    execution_result["failed_actions"].append(
                        {"action_id": action.action_id, "error": str(action_error)}
                    )
            ...
            if not execution_result["success"] and action_plan.retry_instructions:
                retry_result = await self._execute_retry_with_instructions(
                    action_plan.retry_instructions, original_operation, operation_context or {}
                )
            ...
            logger.info(
                f"✅ Action plan execution completed. Success: {execution_result['success']}"
            )
    ...
    async def _execute_single_action(
        self, action: HelpAction, original_operation: Optional[Callable], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            if action.action_type == ActionType.RETRY_OPERATION:
                ...
                return await self._execute_wait_and_retry_action(
                    action, original_operation, context
                )
    ...
    async def _execute_config_update_action(
        self, action: HelpAction, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_retry_with_instructions(
        ...
        if not original_operation:
            return {"success": False, "error": "No original operation for retry instructions"}
    ...
    async def _apply_config_update(self, config_update: ConfigUpdate):
        ...
        logger.info(
            f"📝 Applied config update: {config_update.config_path} = {config_update.new_value}"
        )
...
class AgentHelpActionSystem:
    ...
    async def process_help_and_execute_actions(
        ...
        try:
            ...
            execution_result = await self.executor.execute_action_plan(
                action_plan, original_operation, operation_context
            )
            ...
            return {
                ...
                "resolved_issue": execution_result.get("final_outcome", {}).get(
            ...
            return {"success": False, "error": str(e), "resolved_issue": False}
    ...
    def get_action_statistics(self) -> Dict[str, Any]:
        ...
        successful_executions = sum(
            1 for h in self.action_history if h["execution_result"]["success"]
        )
        resolved_issues = sum(
            1
        ...
        total_actions = sum(
            len(h["execution_result"]["actions_executed"]) for h in self.action_history
        )
...
HelpActionEngine = AgentHelpActionSystem
