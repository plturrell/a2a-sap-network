...
class OpenTelemetryManager:
    ...
    def _init_tracing(self, resource: Resource):
        ...
        # Add OTLP exporter
        otlp_exporter = otlp_trace.OTLPSpanExporter(endpoint=self.config.otlp_endpoint, insecure=True)
    ...
    def _init_metrics(self, resource: Resource):
        ...
        otlp_exporter = otlp_metric.OTLPMetricExporter(endpoint=self.config.otlp_endpoint, insecure=True)
    ...
    def _create_metrics(self):
        ...
        self.error_counter = self.meter.create_counter(
            name="a2a_errors_total", description="Total number of errors", unit="1"
        )

        self.active_tasks_gauge = self.meter.create_up_down_counter(
            name="a2a_active_tasks", description="Number of active tasks", unit="1"
        )

        # Platform sync metrics
        self.platform_sync_counter = self.meter.create_counter(
            name="a2a_platform_syncs_total", description="Total platform synchronizations", unit="1"
        )
    ...
    def record_platform_sync(self, platform: str, status: str, duration: float):
        ...
        attributes = {"platform": platform, "status": status, "agent_id": self.config.agent_id}
...
def record_metric(metric_name: str, value: float, attributes: Optional[Dict[str, Any]] = None):
    ...
    histogram = meter.create_histogram(
        name=metric_name, description=f"Custom metric: {metric_name}", unit="1"
    )
...
class TelemetryEnabledMixin:
    ...
    async def process_message_with_telemetry(
        self, message: Dict[str, Any], context_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.telemetry.record_message_processed(
                message_type=message.get("type", "unknown"), status="success", duration=duration
            )
            ...
            duration = asyncio.get_event_loop().time() - start_time
            self.telemetry.record_message_processed(
                message_type=message.get("type", "unknown"), status="error", duration=duration
            )
            raise
        finally:
            # Update active tasks
