...
class SAPCloudSDK:
    ...
    def _initialize_services(self):
        ...
        ans_client_id = os.getenv("SAP_ANS_CLIENT_ID")
        if ans_client_id:
            ...
            self.services["alert_notification"] = SAPServiceConfig(
                ...
                client_secret=ans_client_secret,
        ...
        if als_client_id:
            ...
            if not all([als_client_secret, als_token_url, als_url]):
                missing = []
                if not als_client_secret:
                    missing.append("SAP_ALS_CLIENT_SECRET")
                if not als_token_url:
                    missing.append("SAP_ALS_TOKEN_URL")
                if not als_url:
                    missing.append("SAP_ALS_URL")
                logger.warning(f"Missing required environment variables: {', '.join(missing)} - Application Logging Service will not be available")
            else:
                self.services["application_logging"] = SAPServiceConfig(
                    service_name="Application Logging",
                    service_url=als_url,
                    client_id=als_client_id,
                    client_secret=als_client_secret,
