...
from .real_vector_db import RealVectorDB
...
class VectorProcessingAgent(A2AAgentBase):
    ...
    async def initialize(self) -> None:
        ...
        if self.vector_db_config.get("use_mock", True):
            logger.info("Using mock vector database for development")
            self.vector_db = MockVectorDB()
            self.vector_db_connected = True
        else:
            # Use real vector database implementation
            logger.info("Initializing real vector database...")
            try:
                self.vector_db = RealVectorDB(self.vector_db_config)
                await self.vector_db.initialize()
                self.vector_db_connected = True
                logger.info("Real vector database connected successfully")
            except Exception as e:
                logger.error(f"Failed to initialize real vector DB: {e}")
                logger.warning("Falling back to mock vector database")
                self.vector_db = MockVectorDB()
                self.vector_db_connected = True
