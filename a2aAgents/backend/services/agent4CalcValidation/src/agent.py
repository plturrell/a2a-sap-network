...
class CalculationValidationAgent(A2AAgentBase):
    ...
    async def _initialize_trust_identity(self) -> None:
        ...
        try:
            ...
            logger.info("Trust identity initialized for Calculation Validation Agent")
    ...
    async def shutdown(self) -> None:
        """Shutdown the Calculation Validation Agent"""
        logger.info("Shutting down Calculation Validation Agent...")
        await self.deregister_from_network()
        
        # Cancel any active validation tasks
        if hasattr(self, 'active_validations'):
