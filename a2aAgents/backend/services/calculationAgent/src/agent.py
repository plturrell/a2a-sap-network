...
class CalculationAgent(A2AAgentBase):
    ...
    async def _initialize_trust_identity(self) -> None:
        ...
        try:
            ...
            logger.info("Trust identity initialized for Calculation Agent")
    ...
    async def shutdown(self) -> None:
        """Shutdown the Calculation Agent"""
        logger.info("Shutting down Calculation Agent...")
        await self.deregister_from_network()
        
        # Cancel any active calculation tasks
        if hasattr(self, 'active_calculations'):
            for calc_id in list(self.active_calculations.keys()):
                calculation = self.active_calculations[calc_id]
                if hasattr(calculation, 'cancel'):
                    calculation.cancel()
