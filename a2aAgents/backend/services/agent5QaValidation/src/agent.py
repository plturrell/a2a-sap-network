...
class QAValidationAgent(A2AAgentBase):
    ...
    async def _initialize_trust_identity(self) -> None:
        """Initialize trust identity for A2A network participation"""
        try:
            # Initialize basic trust identity
            self.trust_identity = {
            ...
            logger.info("Trust identity initialized for QA Validation Agent")
        except Exception as e:
            logger.warning(f"Failed to initialize trust identity: {e} - using fallback")
    ...
    async def shutdown(self) -> None:
        """Shutdown the QA Validation Agent"""
        logger.info("Shutting down QA Validation Agent...")
        await self.deregister_from_network()
        
        # Cancel any active QA validation tasks
        if hasattr(self, 'active_qa_tasks'):
            for task_id in list(self.active_qa_tasks.keys()):
                task = self.active_qa_tasks[task_id]
                task['status'] = 'cancelled'
        
        logger.info("QA Validation Agent shutdown complete")
