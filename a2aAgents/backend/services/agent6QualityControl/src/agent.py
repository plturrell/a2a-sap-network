...
class QualityControlAgent(A2AAgentBase):
    ...
    async def _initialize_trust_identity(self) -> None:
        ...
        try:
            ...
            logger.info("Trust identity initialized for Quality Control Agent")
    ...
    async def shutdown(self) -> None:
        """Shutdown the quality control agent"""
        logger.info("Shutting down Quality Control Agent...")
        await self.deregister_from_network()
        if hasattr(self, 'active_sessions'):
            for session_id in list(self.active_sessions.keys()):
                session = self.active_sessions[session_id]
                session['status'] = QualityStatus.FAILED.value
                session['error'] = "Agent shutdown"
