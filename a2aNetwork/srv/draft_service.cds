...
using a2a.network.draft as draft from '../db/draft-entities';
...
service A2ADraftService @(path: '/api/v1/drafts') {
    ...
    entity Agents as projection on draft.AgentDrafts {
        *,
        ...
        @UI.Hidden: false
        services : Association to many Services on services.provider = $self,
        @UI.Hidden: false
        performance : Association to one AgentPerformance on performance.agent = $self
    } actions {
        @Common.SideEffects: {
    ...
    entity Services as projection on draft.ServiceDrafts {
        *,
        @UI.Hidden: false
        provider : Association to one Agents on provider.ID = $self.provider.ID,
        @UI.Hidden: false
        orders : Composition of many ServiceOrders on orders.service = $self
    } actions {
        @Common.SideEffects: {
    ...
    entity Workflows as projection on draft.WorkflowDrafts {
        *,
        ...
        @UI.Hidden: false
        executions : Association to many WorkflowExecutions on executions.workflow = $self
    } actions {
