...
import { isValidAddress, validateAgentParams } from '../utils/validation';
...
export class AgentManager {
    ...
    async register(params: AgentRegistrationParams): Promise<{ transactionHash: string; agentId: string }> {
        ...
        if (!validation.isValid) {
            throw new A2AError(ErrorCode.INVALID_PARAMS, validation.errors?.join(', ') || 'Invalid parameters');
        }
        ...
        try {
            ...
            const event = receipt.events?.find((e: ContractEvent) => e.event === 'AgentRegistered');
            ...
            throw new A2AError(ErrorCode.REGISTRATION_FAILED, message);
        }
    }
    ...
    async getAgent(agentId: string): Promise<Agent> {
        try {
            ...
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch agent: ${message}`);
        }
    }
        ...
        try {
            ...
            return {
                ...
                reputation: {
                    ...
                    totalEarnings: ethers.formatEther(profileData.totalEarnings)
                },
                performance: {
                ...
                skills: profileData.skillTags.map((tag: string) => ethers.decodeBytes32String(tag))
            };

        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : 'Fetch failed';
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch agent profile: ${message}`);
        }
    }
        ...
        try {
            ...
            if (criteria.skills && criteria.skills.length > 0) {
                ...
                const skillBytes = criteria.skills.map(skill => 
                    ethers.encodeBytes32String(skill)
                );
                ...
                const agents = await Promise.all(
                    result.agents.map(async (address: string, index: number) => {
                        try {
                            ...
                            console.warn('Failed to fetch agent data for search:', {
                                ownerAddress: address,
                                index,
                                error: errorMessage
                            });
                            return null; // Return null to filter out failed agents
                        }
                    })
                );
            ...
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to search agents: ${message}`);
        }
    }
    ...
    async getAllAgents(limit = 50, offset = 0): Promise<{ agents: Agent[]; total: number }> {
        try {
            ...
            const agents = await Promise.all(
                agentIds.map(async (id) => {
                    try {
                        ...
                        const errorMessage = error instanceof Error ? error.message : 'Fetch failed';
                        console.warn('Failed to fetch agent in getAllAgents:', {
                            agentId: id,
                            error: errorMessage
                        });
                        return null; // Return null to filter out failed agents
            ...
            const message = error instanceof Error ? error.message : 'Fetch failed';
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch agents: ${message}`);
        }
    }
    ...
    async getAgentsByOwner(ownerAddress: string): Promise<Agent[]> {
        ...
        try {
            ...
            const agents = await Promise.all(
                agentIds.map(async (id: bigint) => {
                    try {
                        return await this.getAgent(id.toString());
                    } catch (error: unknown) {
                        const errorMessage = error instanceof Error ? error.message : 'Fetch failed';
                        console.warn('Failed to fetch agent by owner:', {
                            ownerAddress,
                            agentId: id.toString(),
                            error: errorMessage
                        });
            ...
            const message = error instanceof Error ? error.message : 'Fetch failed';
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch owner agents: ${message}`);
        }
    }
    ...
    async setStatus(agentId: string, isActive: boolean): Promise<{ transactionHash: string }> {
        try {
            ...
            throw new A2AError(ErrorCode.STATUS_UPDATE_FAILED, message);
        }
    }
    ...
    async deregister(agentId: string): Promise<{ transactionHash: string }> {
        try {
            ...
            throw new A2AError(ErrorCode.DEREGISTRATION_FAILED, message);
        }
    }
    ...
    async getStatistics(agentId: string): Promise<{
        ...
        try {
            ...
            return {
                ...
                earnings: ethers.formatEther(profileData.totalEarnings)
            };

        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : 'Fetch failed';
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch statistics: ${message}`);
        }
    }
     ...
     */
    async subscribeToEvents(callback: (event: Record<string, unknown>) => void): Promise<string> {
     ...
     */
    async subscribeToAgent(agentId: string, callback: (event: Record<string, unknown>) => void): Promise<string> {
        return this.client.subscribe('AgentRegistry', 'AgentUpdated', (id: string, ...args: unknown[]) => {
            if (id === agentId) {
     ...
     */
    private parseCapabilities(capabilities: Array<{ name: string; enabled: boolean }>): AgentCapabilities {
     ...
     */
    async estimateRegistrationGas(params: AgentRegistrationParams): Promise<bigint> {
        try {
            const contract = this.client.getContract('AgentRegistry');
            const capabilitiesArray = Object.entries(params.capabilities).map(([key, value]) => ({
                name: key,
            ...
            return await contract.registerAgent.estimateGas(
                params.name,
                params.description,
                params.endpoint,
                capabilitiesArray,
            ...
            throw new A2AError(ErrorCode.ESTIMATION_FAILED, `Failed to estimate gas: ${message}`);
        }
    }
}