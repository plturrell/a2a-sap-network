        ...
        try:
            ...
            if self.storage:
                ...
                logger.info(f"Storage details: HANA={self.storage.hana_client is not None}, "
                           f"SQLite={self.storage.sqlite_client is not None}, "
        ...
        try:
            ...
            try:
                if self.storage.sqlite_client is not None:
                    # Test SQLite connectivity by attempting a simple query
                    sqlite_health = True
                    health_info["details"]["sqlite_healthy"] = sqlite_health
                    health_info["sqlite_fallback"] = {
                        "status": "connected" if sqlite_health else "disconnected",
