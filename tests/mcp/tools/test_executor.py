    ...
    def _build_test_command(self, test_name: str, suite: TestSuite, coverage: bool) -> List[str]:
        ...
        if suite.type == "unit" or suite.type == "integration":
            ...
            if test_name.endswith(".cy.js"):
                cmd = ["npx", "cypress", "run", "--spec", test_name]
            else:
                cmd = ["npm", "run", "test:e2e", "--", test_name]
        
        elif suite.type == "contracts":
            cmd = ["forge", "test", "--match-test", test_name.replace(".t.sol", "")]
            if coverage:
                cmd.append("--gas-report")
        
        elif suite.type == "performance":
            cmd = ["python", "-m", "pytest", str(self.test_root / "performance" / test_name)]
            cmd.extend(["--benchmark-only", "-v"])
        
        elif suite.type == "security":
            cmd = ["python", "-m", "pytest", str(self.test_root / "security" / test_name)]
