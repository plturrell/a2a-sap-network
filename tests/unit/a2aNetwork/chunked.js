...
const extendInto = require('xtend/mutable');
const expectReadableStream = require('./lib/expect_readable_stream');

const ChunkedHexTransform = require('../chunked_hex_transform');
...
function testChunkedStream(name, options, parts, expectedLines) {
    ...
    test(`ChunkedHexTransform: ${  name}`, (assert) => {
        const stream = PassThrough();
        const trans = ChunkedHexTransform(extendInto({
            encoding: 'utf8'
        }, options));
        stream.pipe(trans);
        parts.forEach((part) => {
        ...
        stream.end();
        const expected = expectedLines.map(expectString);
