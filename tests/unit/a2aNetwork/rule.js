...
const fs = require('fs');
const { Rule } = require(`${PROJECT_DIR}/lib/rule`);
...
suite('rule', function () {
  ...
  //  };
  test('Rule.getSource', () => {
    const src = Rule.getSource('foo:bin/main.o', 'bin/%.o', 'src/%.c');
    assert.equal('foo:src/main.c', src);
  });

  test('rule w/o pattern', () => {
    const out = exec( `${JAKE_CMD} -q  tmp`).toString().trim();
    const output = [
      'tmp_dep2.c task'
      , 'tmp_dep1.c task'
    ...
    const data = fs.readFileSync(`${process.cwd()  }/tmp`);
    assert.equal('src_1src_2', data.toString());
    cleanUpAndNext();
  });

  test('rule w pattern w/o folder w/o namespace', () => {
    const out = exec( `${JAKE_CMD}  -q  tmp_p`).toString().trim();
    const output = [
      'tmp_dep2.c task'
      , 'tmp_dep1.c task'
  ...
  test('rule w pattern w folder w/o namespace', () => {
    const out = exec( `${JAKE_CMD}  -q  tmp_pf`).toString().trim();
    const output = [
  ...
  test.skip('rule w pattern w folder w namespace', () => {
    const out = exec( `${JAKE_CMD} -q   tmp_ns`).toString().trim();
    const output = [
  ...
  test.skip('rule w chain w pattern w folder w namespace', () => {
    const out = exec( `${JAKE_CMD} -q tmp_cr`).toString().trim();
    const output = [
      'chainrule init task'
  ...
  ['precedence', 'regexPattern', 'sourceFunction'].forEach((key) => {

    test(`rule with source file not created yet (${  key   })`, () => {
    ...
    test(`rule with source file now created (${  key   })`, () => {
      fs.writeFileSync('foo.txt', '');
      const out = exec(`${JAKE_CMD} -q  ${  key  }:test`).toString().trim();
    ...
    test(`rule with source file modified (${  key   })`, (next) => {
      setTimeout(() => {
        fs.writeFileSync('foo.txt', '');
        const out = exec(`${JAKE_CMD} -q  ${  key  }:test`).toString().trim();
    ...
    test('rule with existing objective file and no source ' +
        ` (should be normal file-task) (${  key   })`, () => {
