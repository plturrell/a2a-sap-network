...
test('multiple executions', async (t) => {
  const queue = buildQueue(worker, 1);
  const toExec = [1, 2, 3, 4, 5];
  const expected = ['a', 'b', 'c', 'd', 'e'];
  let count = 0;

  await Promise.all(toExec.map(async (task, i) => {
...
test('drained with exception should not throw', async (t) => {
  const queue = buildQueue(worker, 2);
...
test('drained with drain function', async (t) => {
...
test('drained while idle should resolve', async (t) => {
  const queue = buildQueue(worker, 2);
...
test('drained while idle should not call the drain function', async (t) => {
  let drainCalled = false;
  const queue = buildQueue(worker, 2);
...
test('push with worker throwing error', async (t) => {
  t.plan(5);
  const q = buildQueue(async (task, cb) => {
...
test('unshift with worker throwing error', async (t) => {
  t.plan(2);
  const q = buildQueue(async (task, cb) => {
    throw new Error('test error');
  }, 1);
...
test('no unhandledRejection (push)', async (t) => {
...
test('no unhandledRejection (unshift)', async (t) => {
  function handleRejection () {
    t.fail('unhandledRejection');
  }
  process.once('unhandledRejection', handleRejection);
...
test('drained should resolve after async tasks complete', async (t) => {
  const logs = [];
...
test('drained should handle undefined drain function', async (t) => {
  const queue = buildQueue(worker, 1);
