...
// .... /deps/npm
test('test find-node-directory - node install', (t) => {
...
test('test find-node-directory - node build', (t) => {
  t.plan(platforms.length);
  for (let next = 0; next < platforms.length; next++) {
    const processObj = { execPath: '/x/y/bin/node', platform: platforms[next] };
...
test('test find-node-directory - node in bin directory', (t) => {
  t.plan(platforms.length);
  for (let next = 0; next < platforms.length; next++) {
    const processObj = { execPath: '/x/y/bin/node', platform: platforms[next] };
    t.equal(
...
test('test find-node-directory - node in build release dir', (t) => {
  t.plan(platforms.length);
  for (let next = 0; next < platforms.length; next++) {
    var processObj;
    if (platforms[next] === 'win32') {
...
test('test find-node-directory - node in Debug release dir', (t) => {
  t.plan(platforms.length);
  for (let next = 0; next < platforms.length; next++) {
    var processObj;
    if (platforms[next] === 'win32') {
...
test('test find-node-directory - not found', (t) => {
  t.plan(platforms.length);
  for (let next = 0; next < platforms.length; next++) {
