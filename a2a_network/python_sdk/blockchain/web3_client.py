...
from a2a_network.python_sdk.config.contract_config import get_contract_config, ContractConfigManager
...
class A2ABlockchainClient:
    ...
    def __init__(
        ...
        self.rpc_url = rpc_url or os.getenv("A2A_RPC_URL", "http://localhost:8545")
    ...
    def _load_or_create_agent_identity(self, private_key: str = None) -> AgentIdentity:
        ...
        if private_key:
            ...
            if env_key:
                ...
                logger.warning(f"Created new agent identity: {account.address}")
                logger.warning("Save this private key for production use:")
                logger.warning(f"A2A_AGENT_PRIVATE_KEY={account.key.hex()}")
    ...
    def _create_minimal_contract(self, contract_name: str) -> Contract:
        """Create minimal contract instance for development"""
        abi = self._get_minimal_abi(contract_name)
        # Use zero address for development
