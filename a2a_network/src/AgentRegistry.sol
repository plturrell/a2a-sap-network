 ...
 */
contract AgentRegistry is MultiSigPausable, ReentrancyGuard {
     ...
     */
    function increaseReputation(address agent, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function decreaseReputation(address agent, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Agent storage a = agents[agent];
        require(a.owner != address(0), "Not registered");
        uint256 old = a.reputation;
    ...
    function getAgentsPaginated(uint256 offset, uint256 limit) 
        external 
        view 
        returns (address[] memory agentList, uint256 total) 
    {
        ...
        agentList = new address[](length);
        
        for (uint256 i = 0; i < length; i++) {
            agentList[i] = allAgents[offset + i];
        }
    }

    /**
     * @notice Get paginated list of agents by capability
     * @param capability The capability to filter by
     * @param offset Starting index
     * @param limit Maximum number of agents to return
     * @return agentList Array of agent addresses with the capability
     * @return total Total number of agents with the capability
     */
    function getAgentsByCapabilityPaginated(bytes32 capability, uint256 offset, uint256 limit)
        external
        view
        returns (address[] memory agentList, uint256 total)
    {
        uint256 setLength = _capabilityToAgents[capability].length();
        total = setLength;
        ...
        for (uint256 i = 0; i < length; i++) {
            agentList[i] = _capabilityToAgents[capability].at(offset + i);
        }
    }
}
