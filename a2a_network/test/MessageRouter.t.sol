...
contract MessageRouterTest is Test {
    ...
    function setUp() public {
        // Deploy with single confirmation for testing
    ...
    function testRateLimitWindow() public {
        ...
        for (uint256 i = 0; i < router.MAX_MESSAGES_PER_WINDOW(); i++) {
    ...
    function testPauseFunction() public {
        // Only pauser can propose pause
        vm.prank(agent1);
        vm.expectRevert("MultiSigPausable: caller is not a pauser");
        router.proposePause();

        // Pause the contract (single confirmation mode)
        router.proposePause();
        assertTrue(router.paused());

        // Cannot send messages when paused
        vm.prank(agent1);
        vm.expectRevert("MultiSigPausable: paused");
        router.sendMessage(agent2, "Hello", MESSAGE_TYPE_TEXT);

        // Unpause
        router.proposeUnpause();
    ...
    function testUpdateMessageDelay() public {
        uint256 initialDelay = router.messageDelay();
        ...
        // Verify delay unchanged
        assertEq(router.messageDelay(), initialDelay);

        // Update delay as admin
        router.updateMessageDelay(10 seconds);
