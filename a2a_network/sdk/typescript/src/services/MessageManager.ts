...
export class MessageManager {
    ...
    async sendMessage(params: SendMessageParams): Promise<{ 
        ...
        try {
            ...
            if (params.encrypted && params.recipientPublicKey) {
                // Encrypt message
                const encryptedData = await encryptMessage(params.content, params.recipientPublicKey);
            ...
            const tx = await contract.sendMessage(
                ...
                isEncrypted,
                params.replyToId || ethers.constants.HashZero,
                params.metadata || '{}',
                { value: messageFee }
            );
            ...
            throw new A2AError(ErrorCode.SEND_FAILED, error.message);
        }
    }
    ...
    async getMessage(messageId: string): Promise<Message> {
        try {
            ...
            const message: Message = {
                ...
                replyToId: result.replyToId === ethers.constants.HashZero ? undefined : result.replyToId,
                metadata: result.metadata,
                gasUsed: result.gasUsed?.toNumber(),
                fee: result.fee ? ethers.utils.formatEther(result.fee) : undefined
            };
            ...
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch message: ${error.message}`);
        }
    }
    ...
    async getMessages(filter: MessageFilter = {}): Promise<{
        ...
        try {
            ...
            if (filter.type === 'sent') {
                ...
                messageIds = sentIds.map((id: ethers.BigNumber) => id.toString());
            } else if (filter.type === 'received') {
                const receivedIds = await contract.getReceivedMessages(userAddress, offset, limit);
                messageIds = receivedIds.map((id: ethers.BigNumber) => id.toString());
            } else {
                ...
                messageIds = [
                    ...sentIds.map((id: ethers.BigNumber) => id.toString()),
                    ...receivedIds.map((id: ethers.BigNumber) => id.toString())
            ...
            if (error instanceof A2AError) throw error;
            throw new A2AError(ErrorCode.FETCH_FAILED, error.message);
        }
    }
    ...
    async getMessageThread(messageId: string): Promise<MessageThread> {
        try {
            ...
            const messages = await Promise.all(
                threadMessages.map(async (id: ethers.BigNumber) => {
                    try {
            ...
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch thread: ${error.message}`);
        }
    }
    ...
    async deleteMessage(messageId: string): Promise<{ transactionHash: string }> {
        try {
            ...
            if (error instanceof A2AError) throw error;
            throw new A2AError(ErrorCode.DELETE_FAILED, error.message);
        }
    }
    ...
    async getMessageStats(): Promise<{
        ...
        try {
            ...
            for (const message of recentMessages.messages) {
                if (message.replyToId) {
                    try {
                        ...
                        // Skip if original message not found
                    }
                }
            }
            ...
            throw new A2AError(ErrorCode.FETCH_FAILED, `Failed to fetch stats: ${error.message}`);
        }
    }
     ...
     */
    async calculateMessageFee(content: string, priority: string = 'normal'): Promise<ethers.BigNumber> {
        try {
            const contract = this.client.getContract('MessageRouter');
            ...
            throw new A2AError(ErrorCode.CALCULATION_FAILED, `Failed to calculate fee: ${error.message}`);
        }
    }
    ...
    async encryptMessageContent(content: string, recipientPublicKey: string): Promise<EncryptedMessage> {
        return encryptMessage(content, recipientPublicKey);
    }
    ...
    private async tryDecryptMessage(encryptedContent: string): Promise<string | undefined> {
        try {
            // This would require access to user's private key
