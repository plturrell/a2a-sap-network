...
from ..a2a.security.delegation_contracts import get_delegation_contract, can_agent_delegate
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    result = {
                        ...
                        "service_status": health_data.get("status", "running"),
                        "memory_usage": health_data.get("resources", {}).get("memory_usage", "unknown"),
                        "cpu_usage": health_data.get("resources", {}).get("cpu_usage", "unknown"),
                        "active_tasks": health_data.get("active_tasks", 0),
                        "error_rate": health_data.get("error_rate", "0%"),
                        "all_skills_available": health_data.get("capabilities", {}).get("all_available", True),
                        "degraded_skills": health_data.get("capabilities", {}).get("degraded", []),
                        "unavailable_skills": health_data.get("capabilities", {}).get("unavailable", [])
                    }
                else:
                    result = {
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    json={
                        ...
                        "registered_by": f"a2a_registry_{registration.registration_metadata.registered_by}",
                        "tags": ["a2a-agent", "automated"],
                        "labels": {
                ...
                if response.status_code in [200, 201]:
                    logger.info(f"Agent {agent_id} registered in ORD Registry")
                else:
                    logger.warning(f"Failed to register agent {agent_id} in ORD Registry: {response.text}")
            ...
            async with httpx.AsyncClient() as client:
                ...
                if search_response.status_code == 200:
                    ...
                    if results.get("results"):
                        # Deregister from ORD (if such endpoint exists)
                        # For now, just log the action
                        logger.info(f"Agent {agent_id} should be deregistered from ORD Registry")
