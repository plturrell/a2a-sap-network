...
from .workflow_context import workflow_context_manager, WorkflowContext, DataArtifact
from .workflow_monitor import workflow_monitor, WorkflowMetrics, WorkflowState
from app.a2a_trustsystem.service import TrustSystemService
...
def get_trust_service() -> Optional[TrustSystemService]:
    ...
    try:
        from app.a2a_trustsystem.router import get_trust_service as trust_service_factory
...
async def create_workflow(
    ...
    try:
        ...
        initial_artifact = workflow_context_manager.create_data_artifact(
        ...
        try:
            ...
            if trust_service or registry_service:
                workflow_monitor.trust_service = trust_service
                workflow_monitor.registry = registry_service
...
async def get_workflow_status(workflow_id: str):
    ...
    try:
        ...
        context = workflow_context_manager.get_context(workflow_id)
        if not context:
            raise HTTPException(
...
async def get_data_lineage(request: DataLineageRequest):
    ...
    try:
        lineage_artifacts = workflow_context_manager.get_data_lineage(
            request.artifact_id,
            request.workflow_id
        )
...
async def get_workflow_history(hours: int = 24):
    ...
    try:
        history = workflow_monitor.get_workflow_history(hours)
