...
class EnhancedCircuitBreaker:
    ...
    async def initialize(self, health_check_callback: Optional[Callable] = None):
        ...
        try:
            ...
            if REDIS_AVAILABLE:
                ...
                logger.warning(f"Circuit breaker '{self.name}' using local state (Redis not available)")
    ...
    async def _record_failure(self, error: str, duration: float):
        ...
        if self.state == CircuitState.CLOSED:
            ...
            should_open = (
                ...
                failure_rate < self.config.success_threshold or
        ...
        logger.warning(f"Circuit breaker '{self.name}' recorded failure: {error} (duration: {duration:.2f}s)")
    ...
    def _calculate_failure_rate(self) -> float:
        ...
        recent_calls = [
            call for call in self.call_history[-20:]
            if call.timestamp > recent_time
        ]
                ...
                try:
                    ...
                    if self.state == CircuitState.OPEN:
                        ...
                        try:
                            ...
                            if health_result:
                                logger.info(f"Health check passed for '{self.name}', attempting recovery")
                                await self._transition_to_half_open()
...
async def example_agent_communication():
    ...
    # Get circuit breaker
    manager = await get_circuit_breaker_manager()
    cb = await manager.get_circuit_breaker(
        "agent_communication",
        agent_config,
        agent_health_check
    )
