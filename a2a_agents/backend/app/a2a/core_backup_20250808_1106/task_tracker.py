...
class AgentTaskTracker:
    ...
    def create_task(
        ...
        for i, item_desc in enumerate(checklist_items):
            checklist_item = ChecklistItem(
                description=item_desc,
                estimated_duration=estimated_duration // len(checklist_items) if estimated_duration else None
            )
            task.checklist.append(checklist_item)
        ...
        for item in task.checklist:
            if item.item_id == item_id:
                ...
                if not self._are_dependencies_met(task, item):
                    logger.warning(f"⚠️ Cannot start item '{item.description}' - dependencies not met")
                    return False
    ...
    def fail_checklist_item(self, task_id: str, item_id: str, error: str, seek_help: bool = True) -> bool:
    ...
    def receive_help_response(self, request_id: str, response_data: Dict[str, Any]) -> bool:
        ...
        if request_id in self.active_help_requests:
            ...
            if help_request.request_sent_at:
                response_time = (help_request.response_received_at - help_request.request_sent_at).total_seconds()
            ...
            if help_request.checklist_item_id and help_request.task_id in self.tasks:
                ...
                for item in task.checklist:
                    if item.item_id == help_request.checklist_item_id:
                        ...
                        item.notes.append(f"{datetime.utcnow().isoformat()}: Help received from {help_request.target_agent}")
                        break
    ...
    def apply_help_solution(self, request_id: str, effectiveness_rating: int = 5, notes: str = None) -> bool:
                ...
                for item in task.checklist:
                    if item.item_id == help_request.checklist_item_id:
                        if effectiveness_rating >= 3:
                            ...
                            if notes:
                                item.notes.append(f"{datetime.utcnow().isoformat()}: Help applied - {notes}")
                        else:
                            help_request.follow_up_needed = True
                            item.notes.append(f"{datetime.utcnow().isoformat()}: Help ineffective (rating: {effectiveness_rating})")
                        break
    ...
    def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "checklist_summary": {
                ...
                "in_progress": sum(1 for item in task.checklist if item.status == TaskStatus.IN_PROGRESS),
                "waiting_for_help": sum(1 for item in task.checklist if item.status == TaskStatus.WAITING_FOR_HELP),
                "failed": sum(1 for item in task.checklist if item.status == TaskStatus.FAILED)
            },
            "help_requests": {
                "active": len([hr for hr in task.help_requests if hr.status in [HelpRequestStatus.PENDING, HelpRequestStatus.SENT]]),
                "completed": len([hr for hr in task.help_requests if hr.status == HelpRequestStatus.APPLIED])
            },
    ...
    def get_all_tasks_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "help_requests": {
                ...
                "success_rate": (self.performance_metrics["help_requests_successful"] / self.performance_metrics["help_requests_sent"] * 100) 
                    if self.performance_metrics["help_requests_sent"] > 0 else 0
            },
        ...
        return [
            {
                ...
                "waiting_time_minutes": int((datetime.utcnow() - hr.request_sent_at).total_seconds() / 60) 
                    if hr.request_sent_at else 0
            }
    ...
    def _update_help_response_metrics(self, response_time_seconds: float):
        ...
        if total_requests > 1:
            self.performance_metrics["average_help_response_time"] = (
                (current_avg * (total_requests - 1) + response_time_seconds) / total_requests
            )
        else:
    ...
    def _update_task_duration_metrics(self, duration_minutes: float):
        ...
        if completed_tasks > 1:
            self.performance_metrics["average_task_duration"] = (
                (current_avg * (completed_tasks - 1) + duration_minutes) / completed_tasks
            )
        else:
