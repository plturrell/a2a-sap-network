...
from .help_action_engine import AgentHelpActionSystem
from .skill_scope_validator import get_trust_based_help_validator, HelpRequestType
from .task_tracker import TaskStatus
from .message_queue import AgentMessageQueue, MessagePriority, ProcessingMode
...
class AgentHelpSeeker:
    ...
    def __init__(self):
        ...
        self.circuit_breaker = EnhancedCircuitBreaker(
            name="agent_help_communication",
            config=circuit_config
        )
        self.agent_registry = {
            "data_product_agent_0": {
                "url": "http://localhost:8002",
                "specialties": ["data_processing", "ord_registration", "dublin_core", "crd_data"]
            },
            "financial_standardization_agent_1": {
                "url": "http://localhost:8001", 
                "specialties": ["data_standardization", "l4_processing", "integrity_verification", "financial_entities"]
            },
            "data_manager_agent": {
                "url": "http://localhost:8003",
                "specialties": ["data_storage", "crud_operations", "database_management", "file_operations"]
            }
        }
        self.error_counters = {}
    ...
    def _should_seek_help(self, problem_type: str, error_context: Dict[str, Any] = None) -> bool:
        ...
        if failure_count >= self.help_thresholds["consecutive_failures"]:
            logger.info(f"🆘 Seeking help due to {failure_count} consecutive failures for {problem_type}")
            return True
        ...
        if error_context:
            ...
            if "connection" in str(error_context).lower() or "timeout" in str(error_context).lower():
                logger.info(f"🆘 Seeking help due to connectivity issues: {error_context}")
                return True
            
            # Service unavailable
            if "unavailable" in str(error_context).lower() or "service" in str(error_context).lower():
                logger.info(f"🆘 Seeking help due to service issues: {error_context}")
                return True
            
            # Data integrity problems
            if "integrity" in str(error_context).lower() or "corruption" in str(error_context).lower():
    ...
    async def _send_help_request(
        ...
        try:
            ...
            if not validation_result["valid"]:
                logger.warning(f"❌ Help request rejected due to scope violation: {validation_result['reason']}")
                return {
        ...
        try:
            ...
            help_message = A2AMessage(
                ...
                parts=[
                    MessagePart(
                        kind="text",
                        text=f"Help request: {problem_description}"
                    ),
                    MessagePart(
                        kind="data",
                        data={
                            ...
                            "question": f"I'm experiencing {problem_type}: {problem_description}. Can you help me troubleshoot this?"
                        }
                    )
                ]
            )
            ...
            if result is not None:
                ...
                logger.error(f"❌ Help request failed (circuit breaker open or timeout)")
                return None
    ...
    def _determine_help_request_type(self, problem_type: str, context: Dict[str, Any] = None) -> HelpRequestType:
        ...
        if context and context.get("advisor_request"):
            ...
            if any(word in question for word in ["what do you do", "what can you", "capabilities", "skills"]):
                return HelpRequestType.CAPABILITY_QUESTION
            elif any(word in question for word in ["what did you", "how did you", "status", "result"]):
        ...
        if context and ("error" in context or problem_type in ["data_processing", "data_storage", "standardization"]):
    ...
    def get_agent_operational_state(self) -> Dict[str, Any]:
            ...
            current_tasks = [
                {
                    ...
                    "progress": {
                        "completed_items": len([item for item in task.checklist if item.status == TaskStatus.COMPLETED]),
                        "total_items": len(task.checklist),
                        "pending_help_requests": len([item for item in task.checklist if item.help_request_id is not None])
                    }
                }
        ...
        return {
            ...
            "task_summary": {
                ...
                "tasks_waiting_for_help": len([t for t in current_tasks if t["status"] == "waiting_for_help"]),
                "total_checklist_items": sum(t["progress"]["total_items"] for t in current_tasks),
                "completed_checklist_items": sum(t["progress"]["completed_items"] for t in current_tasks)
            },
            "help_activity": {
            ...
            "trust_context": {
                ...
                "known_specialties": list(set().union(*[info["specialties"] for info in self.agent_registry.values()]))
            },
            "message_queue": {
                "queue_enabled": self.message_queue is not None,
                "queue_status": self.message_queue.get_queue_status() if self.message_queue else None
            }
        }