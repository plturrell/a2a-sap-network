...
from app.core.dynamic_config import get_config_manager, validate_production_config
...
from src.a2a.core.router import router as a2a_router
...
from src.a2a.core.workflow_router import router as workflow_router
from app.api.middleware.rate_limiting import rate_limit_middleware
from src.a2a.core.telemetry import init_telemetry, instrument_fastapi, instrument_httpx, instrument_redis
...
try:
    ...
    log_format = '%(asctime)s - %(name)s - %(levelname)s - %(message)s' if monitoring_config.log_format == 'text' else '{"timestamp": "%(asctime)s", "logger": "%(name)s", "level": "%(levelname)s", "message": "%(message)s"}'
    ...
    # Fallback to basic configuration if dynamic config fails
    logging.basicConfig(
        level=logging.INFO,
...
try:
    if config_manager:
        ...
        app.add_middleware(
            CORSMiddleware,
            allow_origins=cors_config.origins if cors_config.origins else ["*"],
