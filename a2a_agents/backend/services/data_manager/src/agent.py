...
class DataManagerAgent(A2AAgentBase):
    ...
    async def _initialize_hana(self) -> None:
        ...
        try:
            ...
            await self._initialize_supabase_fallback()
            
        except Exception as e:
            logger.error(f"Failed to initialize HANA: {e}")
            logger.warning("Falling back to Supabase")
            self.storage_backend = StorageBackend.SUPABASE
            await self._initialize_supabase()
    
    async def _initialize_supabase_fallback(self) -> None:
        """Initialize Supabase as fallback storage"""
        try:
            if self.supabase_url and self.supabase_key:
                self.supabase_client = create_client(self.supabase_url, self.supabase_key)
                logger.info("Supabase fallback initialized")
        except Exception as e:
            logger.warning(f"Could not initialize Supabase fallback: {e}")
    
    async def _initialize_supabase(self) -> None:
        """Initialize Supabase as primary storage (when HANA fails)"""
        try:
            ...
            # Supabase tables should be created via dashboard/migrations
            # Just verify connection
            result = self.supabase_client.table('data_records').select('count').limit(1).execute()
            
            logger.info("Supabase initialized as primary storage")
    ...
    async def _store_record(self, record: DataRecord) -> DataRecord:
        ...
        try:
            ...
            elif self.storage_backend == StorageBackend.SUPABASE:
                # Store in Supabase
                result = self.supabase_client.table('data_records').insert({
            ...
            # Try fallback to Supabase if HANA fails
            if self.storage_backend == StorageBackend.HANA and self.supabase_client:
                ...
                try:
                    ...
                    logger.info("Supabase fallback successful")
                    return record
