...
from .workflow_context import WorkflowContext, WorkflowStage, workflow_context_manager
...
class WorkflowMonitor:
    ...
    async def update_workflow_stage(
        ...
        context = workflow_context_manager.get_context(workflow_id)
        if context:
            workflow_context_manager.update_stage(
                workflow_id, new_stage, agent_id=agent_id
            )
    ...
    async def mark_workflow_timeout(self, workflow_id: str):
        ...
        metrics.end_time = datetime.utcnow()
        metrics.duration_seconds = (metrics.end_time - metrics.start_time).total_seconds()
        metrics.error_message = f"Workflow timed out after {self.workflow_timeouts.get(workflow_id, 0)}s"
    ...
    def get_active_workflows(self) -> List[WorkflowMetrics]:
        ...
        return [
            m for m in self.active_workflows.values()
    ...
    async def cleanup_old_workflows(self, days: int = 7):
        ...
        for wf_id, metrics in self.active_workflows.items():
            if metrics.start_time < cutoff and metrics.state in [
                WorkflowState.COMPLETED, WorkflowState.FAILED, WorkflowState.TIMEOUT
            ]:
