        ...
        try:
            ...
            for ord_id in test_ord_ids:
                try:
                    ...
                    # Clean up Supabase resource index
                    if self.service.storage.supabase_client:
                        result = self.service.storage.supabase_client.client.table("ord_resource_index").delete().eq("ord_id", ord_id).execute()
                        print(f"🗑️ Cleaned Supabase resource index for {ord_id}")
