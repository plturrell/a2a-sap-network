...
from .config.production_config import ProductionConfig, Environment
from .core.telemetry import setup_telemetry
from .agents.agent_manager.active.agent_manager_agent import AgentManager
...
async def start_agent(agent_name: str, port: Optional[int] = None, config_path: Optional[str] = None):
    ...
    if agent_name == "agent0":
        from .agents.agent0_data_product.active.data_product_agent_sdk import DataProductRegistrationAgentSDK
        agent = DataProductRegistrationAgentSDK()
    elif agent_name == "agent1":
        from .agents.agent1_standardization.active.data_standardization_agent_sdk import DataStandardizationAgentSDK
        agent = DataStandardizationAgentSDK()
    elif agent_name == "agent2":
        from .agents.agent2_ai_preparation.active.ai_preparation_agent_sdk import AiPreparationAgentSDK
        agent = AiPreparationAgentSDK()
    elif agent_name == "agent3":
        from .agents.agent3_vector_processing.active.vector_processing_agent_sdk import VectorProcessingAgentSDK
    ...
    logging.info(f"Agent {agent_name} started successfully on port {port}")
    return True
