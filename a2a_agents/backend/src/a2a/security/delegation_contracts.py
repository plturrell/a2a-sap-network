...
from .smart_contract_trust import SmartContractTrust, get_trust_contract
...
class AgentDelegationContract:
    ...
    def create_delegation(
        ...
        try:
            ...
            if trust_score < 0.5:
                raise ValueError(f"Insufficient trust between agents: {trust_score}")
    ...
    def record_delegation_use(
        ...
        try:
            ...
            # Update trust scores based on delegation success
            if success:
                # Successful delegation increases mutual trust
                current_trust = self.trust_contract.get_trust_score(delegator_id, delegatee_id)
                new_trust = min(1.0, current_trust + 0.01)
                if delegator_id in self.trust_contract.trust_relationships:
                    self.trust_contract.trust_relationships[delegator_id][delegatee_id] = new_trust
            else:
                # Failed delegation decreases trust
                current_trust = self.trust_contract.get_trust_score(delegator_id, delegatee_id)
                new_trust = max(0.0, current_trust - 0.05)
                if delegator_id in self.trust_contract.trust_relationships:
                    self.trust_contract.trust_relationships[delegator_id][delegatee_id] = new_trust
                
                logger.warning(f"⚠️ Failed delegation recorded: {delegator_id} → {delegatee_id} ({action})")
