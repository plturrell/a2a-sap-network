...
class PortalService extends cds.ApplicationService {
    ...
    async init() {
        ...
        const { 
            Users, Projects, Agents, Workflows, ProjectFiles, 
            Deployments, TestSuites, ApprovalWorkflows, AuditLogs,
            SystemMetrics, UserSessions
        } = this.entities;
        ...
        this.on('deployAgent', async (req) => {
            ...
            try {
                ...
                const deployment = await this.db.run(
    ...
    async _monitorDeploymentStatus(deploymentId, executionId, agentId, environment, portalUrl) {
        const axios = require('axios');
    ...
    async _monitorWorkflowExecution(executionId, remoteExecutionId, portalUrl) {
        const axios = require('axios');
    ...
    async _executeScriptTask(step, input) {
    ...
    async _getApproversForType(type) {
    ...
    async _getRecentNotifications(userId) {
