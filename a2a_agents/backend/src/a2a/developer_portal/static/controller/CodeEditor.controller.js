sap.ui.define([
    ...
    return Controller.extend("a2a.portal.controller.CodeEditor", {
        ...
        _openFile: function (sPath) {
            ...
            var aOpenTabs = oModel.getProperty("/openTabs");
            
            // Check if already open
            var oExistingTab = aOpenTabs.find(function (tab) {
...
3. Run: \`python -m src.main\``;
        ...
        _updateOutline: function (sLanguage, sContent) {
            ...
            if (sLanguage === "python") {
                ...
                aLines.forEach(function (line, index) {
        ...
        onNavToProjects: function () {
            this._checkUnsavedChanges(function () {
        ...
        onTabSelect: function (oEvent) {
            var sTabId = oEvent.getSource().data("tabId");
            var oModel = this.getView().getModel();
            var aOpenTabs = oModel.getProperty("/openTabs");
            
            var oTab = aOpenTabs.find(function (tab) {
        ...
        onCloseTab: function (oEvent) {
            ...
            var iIndex = aOpenTabs.findIndex(function (tab) {
                return tab.id === sTabId;
            });
            
            if (iIndex > -1) {
                var oTab = aOpenTabs[iIndex];
                ...
                var fnClose = function () {
                    ...
                    if (oModel.getProperty("/activeTab") === sTabId && aOpenTabs.length > 0) {
                        var oNewTab = aOpenTabs[Math.max(0, iIndex - 1)];
                        oModel.setProperty("/activeTab", oNewTab.id);
                        oModel.setProperty("/currentFile", oNewTab);
                    ...
                    MessageBox.confirm("Save changes to " + oTab.name + "?", {
                        onClose: function (sAction) {
                            if (sAction === MessageBox.Action.OK) {
                                this._saveFile(oTab.path, function () {
                                    fnClose();
                                });
                            } else {
                                fnClose();
        ...
        onEditorChange: function (oEvent) {
            ...
            var sActiveTab = oModel.getProperty("/activeTab");
            var aOpenTabs = oModel.getProperty("/openTabs");
            var oTab = aOpenTabs.find(function (tab) {
            ...
            var sLanguage = oModel.getProperty("/editorLanguage");
            this._updateOutline(sLanguage, sContent);
        },

        onSaveFile: function () {
            var oModel = this.getView().getModel();
            var oCurrentFile = oModel.getProperty("/currentFile");
        ...
        _saveFile: function (sPath, fnCallback) {
            var oModel = this.getView().getModel();
            var sContent = oModel.getProperty("/editorContent");
        ...
        onRunFile: function () {
            var oCurrentFile = this.getView().getModel().getProperty("/currentFile");
        ...
        onOutlineSelect: function (oEvent) {
            ...
            var iLine = oContext.getProperty("line");
        ...
        _checkUnsavedChanges: function (fnCallback) {
            var oModel = this.getView().getModel();
            var aOpenTabs = oModel.getProperty("/openTabs");
            var aModifiedTabs = aOpenTabs.filter(function (tab) {
                return tab.modified;
            });
                ...
                MessageBox.confirm("You have unsaved changes. Do you want to save them?", {
                    onClose: function (sAction) {
                        if (sAction === MessageBox.Action.OK) {
                            ...
                            var iSaved = 0;
                            aModifiedTabs.forEach(function (tab) {
                                this._saveFile(tab.path, function () {
                                    iSaved++;
