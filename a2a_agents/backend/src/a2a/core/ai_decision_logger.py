...
class AIDecisionLogger:
    ...
    def __init__(
        ...
        self.storage_path = storage_path or os.path.join(
            os.getenv("AI_DECISION_STORAGE_PATH", "/tmp/ai_decisions"),
            agent_id
        )
    ...
    def _start_background_tasks(self):
        ...
        """Start background analysis and persistence tasks"""
    ...
    def _update_stats(self, decision: AIDecision):
        ...
        stats["avg_response_time"] = (
            (stats["avg_response_time"] * (n - 1) + decision.response_time) / n
        )
        stats["avg_confidence"] = (
            (stats["avg_confidence"] * (n - 1) + decision.confidence_score) / n
        )
    ...
    async def _analyze_patterns(self):
        ...
        for decision_type, stats in self.decision_stats.items():
            if stats["total"] >= 5:  # Minimum sample size
                success_count = sum(
                    1 for outcome in self.outcomes.values()
                    if outcome.decision_id in self.decisions and
                    self.decisions[outcome.decision_id].decision_type.value == decision_type and
        ...
        if len(confidence_outcomes) >= 10:
            ...
            low_conf = [co for co in confidence_outcomes if co["confidence"] < 0.5]
            ...
            if stats["total"] >= 5:
                ...
                if success_rate > 0.8:  # High success with specific context
                    pattern = PatternInsight(
                        ...
                        description=f"Decisions with '{context_key}' context have high success rate",
                        confidence=0.75,
                        evidence_count=stats["total"],
                        success_rate=success_rate,
        ...
        if len(self.learned_patterns) > 50:
            ...
            self.learned_patterns.sort(
                key=lambda p: (p.evidence_count, p.confidence),
                reverse=True
            )
        ...
        logger.info(f"Identified {len(new_patterns)} new patterns, total patterns: {len(self.learned_patterns)}")
    ...
    async def get_recommendations(
        ...
        for pattern in self.learned_patterns:
            ...
            if (decision_type.value in pattern.applicable_contexts or
                "all" in pattern.applicable_contexts):
                
                # Check if context matches
                if context and any(key in pattern.applicable_contexts for key in context.keys()):
                    recommendations.extend(pattern.recommendations)
                elif not context and pattern.pattern_type in ["low_success_rate", "overconfident_failures"]:
                    recommendations.extend(pattern.recommendations)
    ...
    def get_decision_analytics(self) -> Dict[str, Any]:
        ...
        for decision_type in DecisionType:
            ...
            if type_decisions:
                successful = sum(
                    1 for d in type_decisions
                    if d.decision_id in self.outcomes and
                    self.outcomes[d.decision_id].outcome_status == OutcomeStatus.SUCCESS
                )
        ...
        return {
            ...
            "by_type": {
                dt: stats for dt, stats in self.decision_stats.items()
            },
        ...
        for decision_id in reversed(list(self.decision_history)):
            if decision_id in self.decisions:
                ...
                entry = {
                    ...
                    "question": decision.question[:100] + "..." if len(decision.question) > 100 else decision.question,
                    "confidence": decision.confidence_score,
                    "response_time": decision.response_time
                }
        ...
        report = {
            ...
            "data_quality": {
                "decisions_with_outcomes": sum(
                    1 for d_id in self.decisions
                    if d_id in self.outcomes
                ),
...
class AIDecisionRegistry:
    ...
    async def analyze_cross_agent_patterns(self):
        ...
        logger.info(f"Analyzed {len(all_decisions)} decisions across {len(self.agent_loggers)} agents")
    
    def get_global_insights(self) -> Dict[str, Any]:
        """Get insights across all agents"""
        total_decisions = sum(
            logger.performance_metrics["total_decisions"]
            for logger in self.agent_loggers.values()
        )
        
        total_success = sum(
            logger.performance_metrics["successful_outcomes"]
            for logger in self.agent_loggers.values()
        )
        
        agent_performance = {
            agent_id: logger.get_decision_analytics()["summary"]
            for agent_id, logger in self.agent_loggers.items()
        }
